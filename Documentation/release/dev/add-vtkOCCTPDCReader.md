...
### Concerning Assembly/Part Names

vtkDataAssembly only supports node names that follow C variable naming conventions so they must start either with a letter or underscore and can only consist of letters, digits, and underscores.  Since OpenCASCADE/STEP files support more general naming conventions using Unicode characters, the reader will replace all unsupported character sequences with an underscore.

## Concerning vtkPartitionedDataSet Names

Though vtkParationedDatasetCollection does not restrict naming of the datasets comprising it, ParaView's vtkPVDataInformation calls vtkDataAssemblyUtilities::GenerateHierarchy which will build another vtkDataAssembly for the dataset which is a one level tree (i.e. a list).  As a result, all of the vtkPartitionedDataSets' names must be unique; therefore, to avoid any name collisions, the reader will use the default names generated by VTK instead of the names associated with the geometry.
