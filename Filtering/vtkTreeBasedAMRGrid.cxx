/*=========================================================================
  ...
  Module:    vtkTreeBasedAMRGrid.cxx
...
vtkStandardNewMacro(vtkTreeBasedAMRGrid);

vtkCxxSetObjectMacro(vtkTreeBasedAMRGrid,XCoordinates,vtkDataArray);
vtkCxxSetObjectMacro(vtkTreeBasedAMRGrid,YCoordinates,vtkDataArray);
vtkCxxSetObjectMacro(vtkTreeBasedAMRGrid,ZCoordinates,vtkDataArray);

//----------------------------------------------------------------------------
vtkTreeBasedAMRGrid::vtkTreeBasedAMRGrid()
{
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::Cleanup()
{
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::Initialize()
{
...
// object.
void vtkTreeBasedAMRGrid::CopyStructure(vtkDataSet *ds)
{
  vtkTreeBasedAMRGrid *rGrid=static_cast<vtkTreeBasedAMRGrid *>(ds);
  int i;
  this->Initialize();
...
//----------------------------------------------------------------------------
vtkCell *vtkTreeBasedAMRGrid::GetCell(vtkIdType cellId)
{
  vtkCell *cell = NULL;
  vtkIdType idx, npts;
  int loc[3];
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::GetCell(vtkIdType cellId, vtkGenericCell *cell)
{
  vtkIdType idx, npts;
  int loc[3];
  int iMin, iMax, jMin, jMax, kMin, kMax;
...
// constructing a cell.
void vtkTreeBasedAMRGrid::GetCellBounds(vtkIdType cellId, double bounds[6])
{
  int loc[3];
  int iMin, iMax, jMin, jMax, kMin, kMax;
  double x[3];
...
//----------------------------------------------------------------------------
vtkPoints* vtkTreeBasedAMRGrid::GetPoints()
{
...
//----------------------------------------------------------------------------
double *vtkTreeBasedAMRGrid::GetPoint(vtkIdType ptId)
{
  int loc[3];
  ...
  switch (this->DataDescription)
    ...
    default:
      vtkErrorMacro( << "Unexpected value for DataDescription ("
        ...
        << ") in vtkTreeBasedAMRGrid::GetPoint" );
      loc[0] = loc[1] = loc[2] = 0;
      break;
    }
...
void vtkTreeBasedAMRGrid::GetPoint(vtkIdType ptId, double x[3])
{
  int loc[3];
...
//----------------------------------------------------------------------------
vtkIdType vtkTreeBasedAMRGrid::FindPoint(double x[3])
{
  int i, j, loc[3];
  double xPrev, xNext;
...
//
  return 0; // FIXME this->ComputePointId(loc);
}

vtkIdType vtkTreeBasedAMRGrid::FindCell(double x[3], vtkCell *vtkNotUsed(cell), 
                                       ...
                                       double *weights)
{
  ...
  //
  subId = 0;
  return 0; // FIXME this->ComputeCellId(loc);
}

//----------------------------------------------------------------------------
vtkCell *vtkTreeBasedAMRGrid::FindAndGetCell(double x[3],
                                            ...
                                            double pcoords[3], double *weights)
{
  ...
  subId = 0;
  // FIXME  if ( this->ComputeStructuredCoordinates(x, loc, pcoords) == 0 )
    {
    return NULL;
    }
  //
  ...
  // Get the cell
  //
  // FIXME cellId = this->ComputeCellId(loc);

  return vtkTreeBasedAMRGrid::GetCell(cellId);
}

//----------------------------------------------------------------------------
int vtkTreeBasedAMRGrid::GetCellType(vtkIdType vtkNotUsed(cellId))
{
  switch (this->DataDescription)
    {
    case VTK_EMPTY: 
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::ComputeBounds()
{
  double tmp;
...
void vtkTreeBasedAMRGrid::SetDimensions(int i, int j, int k)
{
...
//----------------------------------------------------------------------------
// Set dimensions of rectilinear grid dataset.
void vtkTreeBasedAMRGrid::SetDimensions(int dim[3])
{
  this->SetExtent(0, dim[0]-1, 0, dim[1]-1, 0, dim[2]-1);
}

//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::SetExtent(int extent[6])
{
  int description;
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::SetExtent(int xMin, int xMax, int yMin, int yMax,
                                   int zMin, int zMax)
{
  int extent[6];
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::GetCellNeighbors(vtkIdType cellId, vtkIdList *ptIds,
                                          vtkIdList *cellIds)
{
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::ShallowCopy(vtkDataObject *dataObject)
{
  vtkTreeBasedAMRGrid *grid = vtkTreeBasedAMRGrid::SafeDownCast(dataObject);
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::DeepCopy(vtkDataObject *dataObject)
{
  vtkTreeBasedAMRGrid *grid = vtkTreeBasedAMRGrid::SafeDownCast(dataObject);
...
//----------------------------------------------------------------------------
void vtkTreeBasedAMRGrid::PrintSelf(ostream& os, vtkIndent indent)
{
