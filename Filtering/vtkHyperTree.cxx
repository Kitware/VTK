...
vtkInformationKeyMacro(vtkHyperTree, LEVELS, Integer);
vtkInformationKeyMacro(vtkHyperTree, DIMENSION, Integer);
vtkInformationKeyRestrictedMacro(vtkHyperTree, SIZES, DoubleVector, 3);
...
int vtkHyperTree::GetDataObjectType()
{
  return VTK_DATA_SET;
}
...
// object.
void vtkHyperTree::CopyStructure(vtkDataSet *ds)
{
  assert("pre: ds_exists" && ds!=0);
  assert("pre: same_type" && vtkHyperTree::SafeDownCast(ds)!=0);

  vtkHyperTree *ho=vtkHyperTree::SafeDownCast(ds);
...
void vtkHyperTree::SetDimension(int dim)
{
...
void vtkHyperTree::SetAxisBranchFactor(int factor)
{
  ...
  if (this->AxisBranchFactor == 2)
    ...
    switch(this->Dimension)
      ...
      case 3:
        ...
        this->CellTree=vtkCompactHyperTree<8>::New();
        break;
      case 2:
        this->NumberOfChildren = 4;
        this->CellTree=vtkCompactHyperTree<4>::New();
        break;
      case 1:
        this->NumberOfChildren = 2;
        this->CellTree=vtkCompactHyperTree<2>::New();
        break;
      default:
    ...
    switch(this->Dimension)
      ...
      case 3:
        ...
        this->CellTree=vtkCompactHyperTree<27>::New();
        break;
      case 2:
        this->NumberOfChildren = 9;
        this->CellTree=vtkCompactHyperTree<9>::New();
        break;
      case 1:
        this->NumberOfChildren = 3;
        this->CellTree=vtkCompactHyperTree<3>::New();
        break;
      default:
        assert("check: impossible case" && 0);
        break;
...
//----------------------------------------------------------------------------
void vtkHyperTree::ComputeBounds()
{
...
//-----------------------------------------------------------------------------
// Description:
// Return the number of points corresponding to an hyperTree starting at
// level `level' where all the leaves at at the last level. In this case, the
// hyperTree is like a uniform grid. So this number is the number of points
// of the uniform grid.
// \pre positive_level: level>=0 && level<this->GetNumberOfLevels()
// \post definition: result==(2^(GetNumberOfLevels()-level-1)+1)^GetDimension()
/*
vtkIdType vtkHyperTree::GetMaxNumberOfPoints(int level)
{
  ...
  if (OCTREE)
    {
    result=(1<<(this->GetNumberOfLevels()-level-1))+1;
    }
  else
    {
    int j = (this->GetNumberOfLevels()-level-1);
    while (j-- > 0)
      {
      result *= 3;
      }
...
//-----------------------------------------------------------------------------
// Description:
// Return the number of points corresponding to the boundary of an hyperTree
// starting at level `level' where all the leaves at at the last level. In
// this case, the hyperTree is like a uniform grid. So this number is the
// number of points of on the boundary of the uniform grid.
// For an Tree, the boundary are the faces. For a quadtree, the boundary
// are the edges.
...
/*
vtkIdType vtkHyperTree::GetMaxNumberOfPointsOnBoundary(int level)
{
  assert("Not tested for 27 trees" && 0);
  ...
  if (OCTREE)
    {
    segment=(1<<(this->GetNumberOfLevels()-level-1))+1;
    }
  else
    {
    segment=1;
    int i = (this->GetNumberOfLevels()-level-1);
    while (i-- > 0)
      {
      segment *= 3;
      }
...
// \post positive_result: result>=0
/*
vtkIdType vtkHyperTree::GetMaxNumberOfCellsOnBoundary(int level)
{
  assert("Not tested for 27 trees" && 0);
  assert("pre: positive_level" && level>=0 && level<this->GetNumberOfLevels());
...
// Return the number of levels.
// \post result_greater_or_equal_to_one: result>=1
vtkIdType vtkHyperTree::GetNumberOfLevels()
{
...
// hyperTree cells.
vtkHyperTreeCursor *vtkHyperTree::NewCellCursor()
{
...
// \pre leaf_exists: leaf!=0
// \pre is_a_leaf: leaf->CurrentIsLeaf()
void vtkHyperTree::SubdivideLeaf(vtkHyperTreeCursor *leaf)
{
...
int vtkHyperTree::GetMaxCellSize()
{
  int result;
  switch(this->Dimension)
    {
...
void vtkHyperTree::ShallowCopy(vtkDataObject *src)
{
  assert("src_same_type" && vtkHyperTree::SafeDownCast(src)!=0);
  this->Superclass::ShallowCopy(src);
  this->CopyStructure(vtkHyperTree::SafeDownCast(src));
}

//-----------------------------------------------------------------------------
void vtkHyperTree::DeepCopy(vtkDataObject *src)
{
  assert("src_same_type" && vtkHyperTree::SafeDownCast(src)!=0);
...
//-----------------------------------------------------------------------------
vtkIdType vtkHyperTree::GetNumberOfLeaves()
{
...
// \post positive_result: result>=0
vtkIdType vtkHyperTree::GetNumberOfCells()
{
...
// \post positive_result: result>=0
vtkIdType vtkHyperTree::GetNumberOfPoints()
{
  if (this->DualGridFlag)
    {
    return this->CellTree->GetNumberOfLeaves();
...
// THIS METHOD IS NOT THREAD SAFE.
double *vtkHyperTree::GetPoint(vtkIdType ptId)
{
  if (this->DualGridFlag)
    {
    this->UpdateDualArrays();
...
void vtkHyperTree::GetPoint(vtkIdType id, double x[3])
{
  if (this->DualGridFlag)
    {
    this->UpdateDualArrays();
...
// THIS METHOD IS NOT THREAD SAFE.
vtkCell *vtkHyperTree::GetCell(vtkIdType cellId)
{
  vtkCell *cell = NULL;
...
// THIS METHOD IS THREAD SAFE IF FIRST CALLED FROM A SINGLE THREAD AND
// THE DATASET IS NOT MODIFIED
void vtkHyperTree::GetCell(vtkIdType cellId, vtkGenericCell *cell)
{
  int numPts = 1<<this->GetDimension();
  int ptIdx;
  double x[3];
...
// THIS METHOD IS THREAD SAFE IF FIRST CALLED FROM A SINGLE THREAD AND
// THE DATASET IS NOT MODIFIED
int vtkHyperTree::GetCellType(vtkIdType vtkNotUsed(cellId))
{
  int result;
  switch(this->Dimension)
    {
...
// THIS METHOD IS THREAD SAFE IF FIRST CALLED FROM A SINGLE THREAD AND
// THE DATASET IS NOT MODIFIED
void vtkHyperTree::GetCellPoints(vtkIdType cellId, vtkIdList *ptIds)
{
  int ii;
  int numPts = 1 << this->GetDimension();
  ptIds->Initialize();
...
// method.)
void vtkHyperTree::GetCellPoints(vtkIdType cellId, vtkIdType& npts,
                                        vtkIdType* &pts)
{
  if (this->DualGridFlag)
    {
...
//-----------------------------------------------------------------------------
void vtkHyperTree::GetPointCells(vtkIdType ptId, vtkIdList *cellIds)
{
  vtkIdType *cells;
  int numCells;
  int i;
...
void vtkHyperTree::GetCellNeighbors(vtkIdType cellId, vtkIdList *ptIds,
                                           vtkIdList *cellIds)
{
  int i, j, k;
...
// of a point locator.
vtkIdType vtkHyperTree::FindPoint(double x[3])
{
...
//----------------------------------------------------------------------------
vtkIdType vtkHyperTree::RecursiveFindPoint(double x[3],
  vtkHyperTreeLightWeightCursor* cursor,
...
vtkIdType vtkHyperTree::FindCell(double x[3], vtkCell* cell,
...
//----------------------------------------------------------------------------
vtkIdType vtkHyperTree::FindCell(double x[3], vtkCell *cell, vtkIdType cellId,
                               double tol2, int& subId,double pcoords[3],
                               double *weights)
{
  assert("Not tested for 27 trees" && 0);
...
vtkDataSetAttributes* vtkHyperTree::GetLeafData()
{
  if (this->DualGridFlag)
    {
...
//-----------------------------------------------------------------------------
void vtkHyperTree::SetDualGridFlag(int flag)
{
  if (flag)
    {
    flag = 1;
...
//----------------------------------------------------------------------------
unsigned long vtkHyperTree::GetActualMemorySize()
{
...
//-----------------------------------------------------------------------------
vtkPoints* vtkHyperTree::GetLeafCenters()
{
  this->UpdateDualArrays();
...
//-----------------------------------------------------------------------------
vtkIdTypeArray* vtkHyperTree::GetCornerLeafIds()
{
  this->UpdateDualArrays();
...
void vtkHyperTree::UpdateDualArrays()
{
  ...
  superCursor[midCursorId].Initialize(this);
...
void vtkHyperTree::TraverseDualRecursively(
  ...
  int level)
{
  ...
  if (superCursor[midCursorId].GetIsLeaf())
    ...
    if (superCursor[midCursorId-1].GetTree() && superCursor[midCursorId+1].GetTree())
      ...
      { // Move to maximum boundry of cell
      pt[0] += size[0];
      }
  ...
  // Middle cursor is not leaf. Recurse deeper.
...
void vtkHyperTree::UpdateDualArrays()
{
  ...
  if (this->LeafCenters)
    {
    // If the leaf centers has one point per leaf, assume the arrays
    // have been generated already and are valid.
    if (this->LeafCenters->GetNumberOfPoints() == numLeaves)
      {
      return;
      }
  ...
  this->CornerLeafIds = vtkIdTypeArray::New();
  int dim = this->GetDimension();
  // Number of corner verticies each leaf has.
  int numComps = 1 << dim;
  this->CornerLeafIds->SetNumberOfComponents(numComps);
  ...
  // will traverse the tree as one.
  // Lower dimensions will not use them all.
  vtkHyperTreeLightWeightCursor neighborhood[8];
...
vtkIdType vtkHyperTree::EvaluateGridCorner(
...
//-----------------------------------------------------------------------------
vtkPoints* vtkHyperTree::GetCornerPoints()
{
  this->UpdateGridArrays();
...
//-----------------------------------------------------------------------------
vtkIdTypeArray* vtkHyperTree::GetLeafCornerIds()
{
  this->UpdateGridArrays();
...
//-----------------------------------------------------------------------------
void vtkHyperTree::UpdateGridArrays()
{
  int numLeaves = this->CellTree->GetNumberOfLeaves();
...
void vtkHyperTree::TraverseGridRecursively(
  vtkHyperTreeLightWeightCursor* superCursor,
...
void vtkHyperTree::GenerateSuperCursorTraversalTable()
{
...
//-----------------------------------------------------------------------------
void vtkHyperTree::DeleteInternalArrays()
{
  if (this->LeafCenters)
    {
    this->LeafCenters->Delete();
...
//-----------------------------------------------------------------------------
void vtkHyperTreeLightWeightCursor::Initialize(vtkHyperTree* tree)
{
  //if (this->Tree)
  //  {
  //  this->Tree->UnRegister(0);
...
void vtkHyperTreeLightWeightCursor::ToChild(int child)
{
  ...
  if (this->Tree->Dimension == 3)
    {
    switch (this->Tree->AxisBranchFactor)
      {
      case 2:
        {
        vtkCompactHyperTree<8>* tree3;
        tree3 = static_cast<vtkCompactHyperTree<8>*>(this->Tree->CellTree);
      ...
      case 3:
        ...
        tree3 = static_cast<vtkCompactHyperTree<27>*>(this->Tree->CellTree);
    ...
    switch (this->Tree->AxisBranchFactor)
      ...
      case 2:
        ...
        tree2 = static_cast<vtkCompactHyperTree<4>*>(this->Tree->CellTree);
      ...
      case 3:
        ...
        tree2 = static_cast<vtkCompactHyperTree<9>*>(this->Tree->CellTree);
    ...
    switch (this->Tree->AxisBranchFactor)
      ...
      case 2:
        ...
        tree1 = static_cast<vtkCompactHyperTree<2>*>(this->Tree->CellTree);
      ...
      case 3:
        ...
        tree1 = static_cast<vtkCompactHyperTree<3>*>(this->Tree->CellTree);
...
// It will not work the way it was coded because we used index to detect
// boundary cells.  This is not correct with grid of trees.
// It can iterate over all corners, but cannot generate the dual points
// from the leaves because it is not awary of minimum boundaries.

//----------------------------------------------------------------------------
// This table is used to move a 2x2x2 neighborhood of cursors through the tree.
void vtkHyperTree::GenerateDualNeighborhoodTraversalTable()
{
  int xChild, yChild, zChild;
  int xCursor, yCursor, zCursor;
  int xNeighbor, yNeighbor, zNeighbor;
  ...
  // This should be the same for both octrees and 27trees.
  ...
  switch (this->GetDimension())
    ...
    case 2:
      xChildDim = yChildDim = this->AxisBranchFactor;
      xCursorDim = yCursorDim = 2;
      break;
    case 3:
      xChildDim = yChildDim = zChildDim = this->AxisBranchFactor;
      xCursorDim = yCursorDim = zCursorDim = 2;
      break;
    }
