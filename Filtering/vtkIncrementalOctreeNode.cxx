...
{     
  if ( this->Children )
    ...
    for ( int i = 0; i < 8; i ++ )
      {
      this->Children[i]->Delete(); // the destructor sets the parent to NULL
...
{  
  ...
  if ( fullId == target ) 
    ...
    this->Children[ target ]->UpdateCounterAndDataBoundsRecursively( newPnt, 1 );
