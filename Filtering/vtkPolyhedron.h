...
protected:
  ...
  vtkGenericCell *Cell;
/*
  // Edgetables generated by Clip() and used by Contour(). Define points with
  // values larger/smaller than the contouring value as positive/negative points.
  // Positive edges contain two positive points or one positive one contour point.
  // Negative edges contain two negative points or one negative one contour point.
  // including the new contour edges.
  vtkEdgeTable   *PositiveEdgeTable;
  vtkEdgeTable   *NegativeEdgeTable;
  
  // Map from Face Id to positive/negative points IdArray
  IdToIdVectorMapType   FaceToPositivePointsMap;
  IdToIdVectorMapType   FaceToNegativePointsMap;
*/
  // Map from a face to its points. The points include inserted contouring 
  // points, and are ordered in the point id array.
  IdToIdVectorMapType   FaceToPointsMap;
  
  // Map from a point to its ajacent faces. 
