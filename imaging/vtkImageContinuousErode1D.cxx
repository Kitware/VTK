/*=========================================================================
  ...
  Module:    vtkImageContinuousErode1D.cxx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
//----------------------------------------------------------------------------
// Description:
// Construct an instance of vtkImageContinuousErode1D fitler.
// By default zero values are eroded.
vtkImageContinuousErode1D::vtkImageContinuousErode1D()
{
...
void vtkImageContinuousErode1D::SetFilteredAxis(int axis)
{
  if (axis < 0 || axis > 3)
    {
...
//----------------------------------------------------------------------------
void vtkImageContinuousErode1D::ExecuteImageInformation(vtkImageCache *in, 
							 vtkImageCache *out)
{
  int min, max;
  float spacing;
  ...
  out->SetAxisSpacing(this->FilteredAxis, spacing * (float)(this->Stride));
}
...
//----------------------------------------------------------------------------
void vtkImageContinuousErode1D::ComputeRequiredInputUpdateExtent(
					 vtkImageCache *out, vtkImageCache *in)
{
  int min, max, mid, wholeMin, wholeMax;

  out->GetAxisUpdateExtent(this->FilteredAxis, min, max);
  out->GetAxisWholeExtent(this->FilteredAxis, wholeMin, wholeMax);
...
// and executes the erode algorithm to fill the output from the input.
// Note that input pixel is offset from output pixel.
// It also handles WholeExtent by truncating the kernel.  
template <class T>
static void vtkImageContinuousErode1DExecute(vtkImageContinuousErode1D *self,
			     ...
			     vtkImageRegion *outRegion, T *outPtr)
{
  ...
  for ( ; outIdx <= outMax; ++outIdx)
    {
    *outPtr = *inPtr;
    // The number of pixels cut from the Erode.
...
void vtkImageContinuousErode1D::Execute(vtkImageRegion *inRegion, 
					 vtkImageRegion *outRegion)
{
  ...
  // perform Erode for each pixel of output.
  // Note that input pixel is offset from output pixel.
  ...
  switch (inRegion->GetScalarType())
    {
    case VTK_FLOAT:
      vtkImageContinuousErode1DExecute(this, inRegion, (float *)(inPtr), 
				 outRegion, (float *)(outPtr));
      break;
    case VTK_INT:
      vtkImageContinuousErode1DExecute(this, inRegion, (int *)(inPtr),
				 outRegion, (int *)(outPtr));
      break;
    case VTK_SHORT:
      vtkImageContinuousErode1DExecute(this, inRegion, (short *)(inPtr),
				 outRegion, (short *)(outPtr));
      break;
    case VTK_UNSIGNED_SHORT:
      vtkImageContinuousErode1DExecute(this,
