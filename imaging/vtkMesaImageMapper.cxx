...
//------------------------------------------------------------------------------
vtkMesaImageMapper* vtkMesaImageMapper::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkMesaImageMapper");
  if(ret)
    {
    return (vtkMesaImageMapper*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkMesaImageMapper;
}




vtkMesaImageMapper::vtkMesaImageMapper()
{
}
...
template <class T>
static void vtkMesaImageMapperRender(vtkMesaImageMapper *self, 
				       vtkImageData *data, 
				       T *dataPtr,
...
template <class T>
static void vtkMesaImageMapperRenderShort(vtkMesaImageMapper *self, 
					    vtkImageData *data, 
					    T *dataPtr,
					    float shift, float scale,
					    int *actorPos, int *vsize)
				     ...
				     vtkImageData *data, vtkActor2D *actor)
{
  ...
  if (!window)
    {
    vtkErrorMacro (<<"vtkMesaImageMapper::RenderData - no window set for viewport");
    return;
    }
