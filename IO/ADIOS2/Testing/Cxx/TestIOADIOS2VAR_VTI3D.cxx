...
template <class T>
bool CompareData(const std::string &name, vtkImageData *imageData,
...
private:
    ...
    bool DoCheckData(vtkMultiBlockDataSet *multiBlock)
    {
        ...
        vtkImageData *imageData =
            vtkImageData::SafeDownCast(multiPiece->GetPiece(rank));
...
void WriteBPFile3DVars(const std::string &fileName, const adios2::Dims &shape,
                       ...
                       const size_t steps, const int rank,
                       const bool isAttribute, const bool hasTime,
                       const bool isCellData = true)
{
    ...
    const std::string extent = "0 " + std::to_string(Nx) + " " + "0 " +
                               std::to_string(Ny) + " " + "0 " +
                               std::to_string(Nz);
                       ...
                       const bool isCellData = true)
{
   ...
   <DataArray Name="TIME">
    ...
    const size_t linearStart =
        start[0] * shape[1] * shape[2] + start[1] * shape[2] + start[2];
...
int TestIOADIOS2VAR_VTI3D(int argc, char *argv[])
{
    ...
    const adios2::Dims shape{static_cast<size_t>(size) * count[0], count[1],
                             count[2]};
