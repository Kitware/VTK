...
public:
  ...
  const bool GetIs13Positions() const
  {
...
void vtkOpenFOAMReaderPrivate::vtkFoamFile::ThrowUnexpectedNondigitCharExecption(
    const int c)
{
...
template<> void vtkOpenFOAMReaderPrivate::vtkFoamEntryValue::listTraits<
    vtkFloatArray, float>::ReadBinaryList(vtkFoamIOobject& io, const int size)
{
  for (int i = 0; i < size; i++)
    {
    double buffer;
...
vtkOpenFOAMReaderPrivate::vtkFoamIntVectorVector * vtkOpenFOAMReaderPrivate::ReadFacesFile(
...
vtkOpenFOAMReaderPrivate::vtkFoamIntVectorVector * vtkOpenFOAMReaderPrivate::ReadOwnerNeighborFiles(
...
{
  for (size_t boundaryI = 0, activeBoundaryI = 0; boundaryI
      < this->BoundaryDict.size(); boundaryI++)
    {
