...
std::string vtkFLUENTCFFInternal::GetMatchingFieldName(const std::string& strSectionName)
{
  ...
  return vtkFLUENTCFFInternal::FieldsNamesMap().at(fieldName) + vector_string +
    previous_step_string + " (" + strSectionName + ")";
}

const std::unordered_map<std::string, std::string>& vtkFLUENTCFFInternal::FieldsNamesMap()
{
  static const std::unordered_map<std::string, std::string> fieldnames = { { "R", "density" },
    ...
    { "RUU", "uu Reynolds stress" }, { "RVV", "vv Reynolds stress" },
    { "RWW", "ww Reynolds stress" }, { "RUV", "uv Reynolds stress" },
    { "RVW", "vw Reynolds stress" }, { "RUW", "uw Reynolds stress" }, { "VOF", "volume fraction" },
    { "PHI", "potential value" }, { "ELEC_COND", "electrical conductivity" },
    ...
    { "TP_VFF", "volatile fraction remaining" },
    { "DPM_BOILING_TEMPERATURE", "boiling temperature" },
    ...
    { "DPMS_SPECIES", "DPM species mass" },
    { "DPMS_VAP_PER_MAT_0", "DPM vaporization per material" }, { "ENERGY", "energy" },
    { "MU_LAM", "laminar dynamic viscosity" }, { "PHASE_MASS", "phase mass in multiphase flow" },
    { "THIN_FILM", "thin film model quantity" }, { "WALL_DIST", "distance to nearest wall" },
    { "Y", "mass fraction" } };
  return fieldnames;
}
