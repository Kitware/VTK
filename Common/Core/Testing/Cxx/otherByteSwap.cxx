...
{
  // actual test
  strm << "Test vtkByteSwap Start" << std::endl;
  ...
  strm << "Swap2BE(short \"ab\") -> " << check[0] << check[1] << std::endl;

  memcpy(usword, "abcd", 2);
  vtkByteSwap::Swap2BE(usword);
  memcpy(check, usword, 2);
  strm << "Swap2BE(unsigned short \"ab\") -> " << check[0] << check[1] << std::endl;

  memcpy(cword, "abcd", 4);
  vtkByteSwap::Swap4BE(cword);
  memcpy(check, cword, 4);
  strm << "Swap4BE(char *\"abcd\") -> " << check[0] << check[1] << check[2] << check[3]
       << std::endl;
  ...
  strm << "Swap8BE(char *\"abcdefgh\") -> " << check[0] << check[1] << check[2] << check[3]
       << check[4] << check[5] << check[6] << check[7] << std::endl;
  ...
  memcpy(check, sword, 2);
  strm << "Swap2LE(short \"ab\") -> " << check[0] << check[1] << std::endl;

  memcpy(usword, "abcd", 2);
  vtkByteSwap::Swap2LE(usword);
  memcpy(check, usword, 2);
  strm << "Swap2LE(unsigned short \"ab\") -> " << check[0] << check[1] << std::endl;

  memcpy(cword, "abcd", 4);
  vtkByteSwap::Swap4LE(cword);
  memcpy(check, cword, 4);
  strm << "Swap4LE(char *\"abcd\") -> " << check[0] << check[1] << check[2] << check[3]
       << std::endl;
  ...
  strm << "SwapVoidRange(char *\"abcdefgh\",4,2) -> " << check[0] << check[1] << check[2]
       << check[3] << check[4] << check[5] << check[6] << check[7] << std::endl;
  ...
  strm << "Test vtkByteSwap End" << std::endl;
  return 0;
}
