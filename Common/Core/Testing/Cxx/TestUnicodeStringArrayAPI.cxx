...
static const vtkUnicodeString sample_utf8_greek =
  vtkUnicodeString::from_utf8("\xce\xb1\xce\xb2\xce\xb3"); // Greek lower-case alpha, beta, gamma.
static const vtkUnicodeString sample_utf8_thai = vtkUnicodeString::from_utf8(
  "\xe0\xb8\x81\xe0\xb8\x82\xe0\xb8\x83"); // Thai ko kai, kho khai, kho khuat.
static const vtkUnicodeString sample_utf8_linear_b = vtkUnicodeString::from_utf8(
  "\xf0\x90\x80\x80\xf0\x90\x80\x81\xf0\x90\x80\x82\xf0\x90\x80\x83\xf0\x90\x80\x84"); // Linear-B
                                                                                       // syllables
                                                                                       // a, e, i,
                                                                                       // o, u.
static const vtkUnicodeString sample_utf8_mixed = vtkUnicodeString::from_utf8(
...
int TestUnicodeStringArrayAPI(int, char*[])
{
  try
  {
    ...
    vtkSmartPointer<vtkUnicodeStringArray> array2 = vtkSmartPointer<vtkUnicodeStringArray>::New();
    ...
    vtkSmartPointer<vtkUnicodeStringArray> array3 = vtkSmartPointer<vtkUnicodeStringArray>::New();
    ...
    vtkSmartPointer<vtkIdList> toIds = vtkSmartPointer<vtkIdList>::New();
    vtkSmartPointer<vtkIdList> fromIds = vtkSmartPointer<vtkIdList>::New();
    ...
    array3->InsertNextValue(vtkUnicodeString::from_utf8("foobar"));
    array3->InsertNextValue(vtkUnicodeString::from_utf8("foobar"));
    vtkSmartPointer<vtkIdList> lookupIds = vtkSmartPointer<vtkIdList>::New();
    ...
    vtkSmartPointer<vtkUnicodeStringArray> array4 = vtkSmartPointer<vtkUnicodeStringArray>::New();
    ...
    vtkSmartPointer<vtkUnicodeStringArray> array5 = vtkSmartPointer<vtkUnicodeStringArray>::New();
    ...
    vtkSmartPointer<vtkIdList> interpIds = vtkSmartPointer<vtkIdList>::New();
    ...
    array3->InterpolateTuple(0, 0, array4, 0, array5, 0.1);
    test_expression(array3->GetValue(0) == array4->GetValue(0));

    array3->InterpolateTuple(1, 0, array4, 0, array5, 0.6);
...
int TestErrorsAndWarnings()
{
  ...
  {
    std::cout << "Caught expected warning: " << errorObserver->GetWarningMessage();
  }
  else
  {
    std::cout
      << "Failed to catch expected 'Input and output array data types do not match.' warning"
      << std::endl;
    ++status;
  }
  errorObserver->Clear();
  ...
  // Warning: Input and output array data types do not match.
  vtkSmartPointer<vtkIdList> id1 = vtkSmartPointer<vtkIdList>::New();
  ...
  vtkSmartPointer<vtkIdList> id2 = vtkSmartPointer<vtkIdList>::New();
  ...
  {
    std::cout << "Failed to catch expected 'Input and output id array sizes do not match.' warning"
              << std::endl;
    ++status;
  }
  errorObserver->Clear();
  ...
  if (errorObserver->GetError())
  {
    std::cout << "Caught expected warning: " << errorObserver->GetErrorMessage();
  }
  else
  {
    std::cout << "Failed to catch expected 'Cannot CopyValue from array of type' error"
              << std::endl;
    ++status;
  }
  errorObserver->Clear();
  ...
  array->InterpolateTuple(0, 0, doubleArray, 2, array, 0.0);
  if (errorObserver->GetError())
  {
    std::cout << "Caught expected warning: " << errorObserver->GetErrorMessage();
  }
  ...
  {
    std::cout
      << "Failed to catch expected 'All arrays to InterpolateValue() must be of same type.' error"
      << std::endl;
    ++status;
  }
  errorObserver->Clear();
