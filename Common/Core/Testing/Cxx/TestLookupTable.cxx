...
{
  int failed = expected[0] != test[0] || expected[1] != test[1] || expected[2] != test[2] ||
      expected[3] != test[3]
    ? 1
    : 0;
  if (failed)
  {
    std::cerr << "Expected color: " << static_cast<int>(expected[0]) << ", "
              << static_cast<int>(expected[1]) << ", " << static_cast<int>(expected[2]) << ", "
              << static_cast<int>(expected[3]) << std::endl;
    std::cerr << "Test color: " << static_cast<int>(test[0]) << ", " << static_cast<int>(test[1])
              << ", " << static_cast<int>(test[2]) << ", " << static_cast<int>(test[3])
              << std::endl;
  }
...
public:
  ...
  void Callback(vtkObject*, unsigned long e, void*) { this->EventId = e; }
...
int TestLookupTable(int, char*[])
{
  ...
  vtkMath::HSVToRGB(table->GetHueRange()[0], table->GetSaturationRange()[0],
    table->GetValueRange()[0], &rgba[0], &rgba[1], &rgba[2]);
  ...
  vtkMath::HSVToRGB(table->GetHueRange()[1], table->GetSaturationRange()[1],
    table->GetValueRange()[1], &rgba[0], &rgba[1], &rgba[2]);
  ...
  unsigned long observerId =
    table->AddObserver(vtkCommand::ErrorEvent, &observer, &errorObserver::Callback);
