...
//----------------------------------------------------------------------------
void vtkPentagonalPrism::Contour(double value, vtkDataArray* cellScalars, 
                                 ...
                                 vtkCellData *outCd)
{
  ...
  // of the 2 pentagonal faces
  this->Subdivide(inPd, inCd, cellId);

  vtkDoubleArray *newCellScalars=vtkDoubleArray::New();
  newCellScalars->SetNumberOfValues(6);

  s1 = s2 = 0.;
  for(i=0; i<5; ++i)
    {
    s1 +=  cellScalars->GetComponent(i,0);
    s2 +=  cellScalars->GetComponent(i+5,0);
    }
  s1 *= 0.2;
  s2 *= 0.2;
  newCellScalars->SetValue(2, s1);
  newCellScalars->SetValue(5, s2);
 ...
 //contour each internal wedge separately
  for(int i=0; i<5; i++) //for each wedge
    ...
    for(int j=0; j<6; j++)
      {
      this->Wedge->Points->SetPoint(j, this->Points->GetPoint(InternalWedges[i][j]));
      this->Wedge->PointIds->SetId(j,InternalWedges[i][j]);  
      if(j!=2 && j!=5)
        {
    ...
    this->Wedge->Contour(value, newCellScalars, locator, verts, lines, polys, 
                         this->PointData, outPd, this->CellData, 0, outCd);
    }
  newCellScalars->Delete();
}
//----------------------------------------------------------------------------
void vtkPentagonalPrism::Clip(double value, vtkDataArray *cellScalars, 
                              ...
                              vtkCellData *outCd, int insideOut)
{
  ...
  for(int i=0; i<5; i++) //for each wedge
    ...
    for(int j=0; j<6; j++)
      {
      this->Wedge->Points->SetPoint(j,
                this->Points->GetPoint(InternalWedges[i][j]));
      this->Wedge->PointIds->SetId(j,InternalWedges[i][j]);
      if(j!=2 && j!=5)
        {
        newCellScalars->SetValue(j, cellScalars->GetTuple1(InternalWedges[i][j]));
        }
      this->Scalars->SetValue(j, localScalars->GetTuple1(InternalWedges[i][j]));
      }
    this->Wedge->Clip(value, newCellScalars, locator, tets, this->PointData, 
                      outPd, this->CellData, cellId, outCd, insideOut);
    }
  newCellScalars->Delete();
}
//----------------------------------------------------------------------------
void vtkPentagonalPrism::Subdivide(vtkPointData *inPd, vtkCellData *inCd, vtkIdType cellId)
{
  int i, j;
  double x[3], x1[3], x2[3], s1, s2, weights[5];

  this->PointData->CopyAllocate(inPd, 10+2);
  ...
  for(i=0; i<5; ++i)
    ...
    this->Points->GetPoint(i+5, x);
    for(j=0; j<3; ++j)
      {
      x2[j] += x[j]*weights[i];
      }
    }
  this->PointData->InterpolatePoint(inPd, 10, GetFace(0)->GetPointIds(), weights);
  this->PointData->InterpolatePoint(inPd, 11, GetFace(1)->GetPointIds(), weights);

  this->Points->SetPoint(10, x1); 
  this->Points->SetPoint(11, x2);
