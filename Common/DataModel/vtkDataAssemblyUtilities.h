...
public:
   ...
   *
   * The generated hierarchy contains the needed attributes to convert output
   * back to the original vtkCompositeDataSet subclass using
  ...
  ///@{
  /**
   * Inverse of `GenerateHierarchy`. Given a PDC (vtkPartitionedDataSetCollection) and
   ...
   *
   * It should be used to convert an input PDC generated from `GenerateHierarchy`,
   * as it relies on some specific hierarchy attributes provided by this method.
   * So for now it can return the following types:
   ...
   * valid.
   *
   * @see HasCompatibilityHierarchy, GenerateHierarchy
   */
  ...
  ///@{
  /**
   * Return true if the PartitionedDataSetCollection has a hierarchy
   * that allows a conversion back to another vtkCompositeDataSet subclass,
   * as done in `GenerateCompositeDataSetFromHierarchy`.
   *
   * It expects some specific attributes as generated by `GenerateHierarchy`.
   * The hierarchy should have been generated from a vtkUniformGridAMR subclass
   * or from a vtkMultiBlockDataSet.
   */
  static bool HasCompatibilityHierarchy(vtkPartitionedDataSetCollection* input);
  static bool IsCompatibilityHierarchy(vtkDataAssembly* hierarchy);
  ///@}
