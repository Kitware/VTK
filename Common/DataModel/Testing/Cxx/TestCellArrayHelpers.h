...
void TestLegacyGetInsertLocation(vtkSmartPointer<vtkCellArray> cellArray)
{
  ...
  std::array<vtkIdType, 4> locs{};
  for (int i = 1; i < 4; ++i)
  {
    auto ids = GetCellIds<FixedSize>(i - 1);
    locs[i] = locs[i - 1] + ids->GetNumberOfIds() + 1;
  }
  for (vtkIdType i = 0; i < 3; ++i)
  {
    auto ids = GetCellIds<FixedSize>(i);
    cellArray->InsertNextCell(ids);
    TEST_ASSERT(cellArray->GetInsertLocation(ids->GetNumberOfIds()) == locs[i]);
  }
}
...
void TestLegacyGetSetTraversalLocation(vtkSmartPointer<vtkCellArray> cellArray)
{
  ...
  for (int i = 1; i < 4; ++i)
  {
    ...
    TEST_ASSERT(cellArray->GetTraversalLocation() == locs[i]);
    TEST_ASSERT(cellArray->GetTraversalLocation(ids->GetNumberOfIds()) == locs[i - 1]);
  }

  for (int i = 0; i < 3; ++i)
  {
    cellArray->SetTraversalLocation(locs[i]);
    TEST_ASSERT(cellArray->GetTraversalCellId() == i);
  }
}
...
void TestLegacyReverseCell(vtkSmartPointer<vtkCellArray> cellArray)
{
  ...
  cellArray->ReverseCell(locs[2]);

  validate(0, false);
  validate(1, false);
  validate(2, true);

  cellArray->ReverseCell(locs[0]);
...
void TestLegacyReplaceCell(vtkSmartPointer<vtkCellArray> cellArray)
{
  ...
  {
    auto ids2Reverse = GetCellIds<FixedSize>(2, true);
    cellArray->ReplaceCell(locs[2], ids2Reverse->GetNumberOfIds(), ids2Reverse->GetPointer(0));
  }
  ...
  {
    auto ids1 = GetCellIds<FixedSize>(1, true);
    cellArray->ReplaceCell(locs[1], ids1->GetNumberOfIds(), ids1->GetPointer(0));
  }
  ...
  {
    auto ids0 = GetCellIds<FixedSize>(0, true);
    cellArray->ReplaceCell(locs[0], ids0->GetNumberOfIds(), ids0->GetPointer(0));
  }
...
void TestLegacySetCells(vtkSmartPointer<vtkCellArray> cellArray)
{
  ...
  {
    auto ids = GetCellIds<FixedSize>(i);
    legacy->InsertNextValue(ids->GetNumberOfIds());
    for (vtkIdType j = 0; j < ids->GetNumberOfIds(); ++j)
    {
      legacy->InsertNextValue(ids->GetId(j));
    }
  }
...
void RunTests()
{
  ...
  TestLegacyGetCell<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacyGetInsertLocation<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacyGetSetTraversalLocation<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacyReverseCell<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacyReplaceCell<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacyGetData<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
  TestLegacySetCells<FixedSize>(CellArrayFactory<StorageType, ArrayTypes...>::New());
}
