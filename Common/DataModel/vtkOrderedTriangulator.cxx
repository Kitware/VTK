...
{
  void* operator new(size_t size, vtkHeap* heap) { return heap->AllocateMemory(size); }
...
struct OTTetra
{
    ...
    , Type(OutsideCavity)
  {
    ...
    this->Neighbors[0] = this->Neighbors[1] = this->Neighbors[2] = this->Neighbors[3] = nullptr;
    this->DeleteMe = 0;
  }
...
struct vtkOTMesh
{
  vtkOTMesh(vtkHeap* heap)
    : NumberOfTetrasClassifiedInside(0)
    , NumberOfTemplates(0)
  {
    this->EdgeTable = vtkEdgeTable::New();
    this->Heap = heap;
  }
  ~vtkOTMesh() { this->EdgeTable->Delete(); }
...
struct OTTemplate
{
  ...
  OTTemplate(vtkIdType numberOfTetras, vtkHeap* heap)
  {
    ...
    this->Tetras =
      static_cast<vtkIdType*>(heap->AllocateMemory(sizeof(vtkIdType) * numberOfTetras * 4));
  }
  void* operator new(size_t size, vtkHeap* heap) { return heap->AllocateMemory(size); }
  void operator delete(void*, vtkHeap*) {}
};
...
vtkIdType vtkOrderedTriangulator::InsertPoint(vtkIdType id, double x[3], double p[3], int type)
{
  ...
  this->Mesh->Points[idx].Type = static_cast<OTPoint::PointClassification>(type);
...
vtkIdType vtkOrderedTriangulator::InsertPoint(
  vtkIdType id, vtkIdType sortid, double x[3], double p[3], int type)
{
  vtkIdType idx = this->NumberOfPoints++;
  if (idx >= this->MaximumNumberOfPoints)
  {
...
{
  assert("pre: valid_range" && internalId >= 0 && internalId < this->NumberOfPoints);
  this->Mesh->Points[internalId].Type = static_cast<OTPoint::PointClassification>(type);
}
...
{
  if ((this->Points[0]->Type == OTPoint::Inside || this->Points[0]->Type == OTPoint::Boundary) &&
    (this->Points[1]->Type == OTPoint::Inside || this->Points[1]->Type == OTPoint::Boundary) &&
    (this->Points[2]->Type == OTPoint::Inside || this->Points[2]->Type == OTPoint::Boundary) &&
    (this->Points[3]->Type == OTPoint::Inside || this->Points[3]->Type == OTPoint::Boundary))
  {
    ...
    (this->Points[1]->Type == OTPoint::Outside || this->Points[1]->Type == OTPoint::Boundary) &&
    (this->Points[2]->Type == OTPoint::Outside || this->Points[2]->Type == OTPoint::Boundary) &&
    (this->Points[3]->Type == OTPoint::Outside || this->Points[3]->Type == OTPoint::Boundary))
  {
...
OTTetra* vtkOTMesh::CreateTetra(OTPoint* p, OTFace* face)
{
  ...
  tetra->Radius2 = vtkTetra::Circumsphere(
    p->P, face->Points[0]->P, face->Points[1]->P, face->Points[2]->P, tetra->Center);
...
int vtkOTMesh::CreateInsertionCavity(OTPoint* p, OTTetra* initialTet, double[4])
{
  ...
  for (int numCycles = 0; !this->TetraStack.empty(); numCycles++)
  {
    ...
    if (!valid) // broke out due to invalid face
    {
      ...
      for (titer = this->DegenerateQueue.begin(); titer != this->DegenerateQueue.end(); ++titer)
      {
  ...
  {
    tetra = *t;
    if (tetra->CurrentPointId == p->InsertionId && tetra->Type == OTTetra::InCavity)
    {
      tetra->DeleteMe = 1;
    }
  }
...
OTTetra* vtkOTMesh::WalkToTetra(OTTetra* tetra, double x[3], int depth, double bc[4])
{
  ...
  vtkTetra::BarycentricCoords(
    x, tetra->Points[0]->P, tetra->Points[1]->P, tetra->Points[2]->P, tetra->Points[3]->P, bc);
...
void vtkOrderedTriangulator::Triangulate()
{
  ...
  if (!this->PreSorted)
  {
    ...
    {
      qsort(
        this->Mesh->Points.GetPointer(0), this->NumberOfPoints, sizeof(OTPoint), vtkSortOnTwoIds);
    }
    else
    {
      qsort(this->Mesh->Points.GetPointer(0), this->NumberOfPoints, sizeof(OTPoint), vtkSortOnIds);
    }
  }
  ...
  // as we progress.
  for (ptId = 0, p = this->Mesh->Points.GetPointer(0); ptId < this->NumberOfPoints; ++p, ++ptId)
  {
    ...
    double bc[4];
    OTTetra* tetra = this->Mesh->WalkToTetra(*(this->Mesh->Tetras.begin()), p->P, 0, bc);
    ...
    for (fptr = this->Mesh->CavityFaces.begin(); fptr != this->Mesh->CavityFaces.end(); ++fptr)
    {
      face = *fptr;
...
//------------------------------------------------------------------------
// Add the tetras classified as specified to an unstructured grid
vtkIdType vtkOrderedTriangulator::AddTetras(int classification, vtkCellArray* outConnectivity)
{
  TetraListIterator t;
  OTTetra* tetra;
  vtkIdType numTetras = 0;
...
vtkIdType vtkOrderedTriangulator::AddTetras(int classification, vtkIncrementalPointLocator* locator,
...
int vtkOrderedTriangulator::GetNextTetra(
...
vtkIdType vtkOrderedTriangulator::AddTriangles(vtkCellArray* tris)
{
  ...
  for (t = this->Mesh->Tetras.begin(); t != this->Mesh->Tetras.end(); ++t)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      if (tetra->Neighbors[i] && tetra->Neighbors[i]->CurrentPointId != VTK_INT_MAX &&
...
vtkIdType vtkOrderedTriangulator::AddTriangles(vtkIdType id, vtkCellArray* tris)
{
  ...
  for (t = this->Mesh->Tetras.begin(); t != this->Mesh->Tetras.end(); ++t)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      { // face not yet visited
        tetra->GetFacePoints(i, face);
        if (face->Points[0]->Id == id || face->Points[1]->Id == id || face->Points[2]->Id == id)
        {
          numTris++;
          tris->InsertNextCell(3);
          tris->InsertCellPoint(face->Points[0]->Id);
...
{
  static TemplateIDType mask[8] = { 0xF0000000, 0x0F000000, 0x00F00000, 0x000F0000, 0x0000F000,
