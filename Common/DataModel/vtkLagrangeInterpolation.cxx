...
int vtkLagrangeInterpolation::Tensor3ShapeDerivatives(const int order[2], const double pcoords[3], double* deriv)
{
  // FIXME: Eventually needs to be varying length.
  double ll[3][vtkLagrangeInterpolation::MaxDegree + 1];
...
void vtkLagrangeInterpolation::WedgeShapeDerivatives(const int order[2], const double pcoords[3], double* derivs)
{
  static vtkNew<vtkLagrangeTriangle> tri;
    ...
    for (int jj = 0; jj <= rsOrder; ++jj)
    {
      ...
      for (int ii = 0; ii <= rsOrder - jj; ++ii)
      {
        ...
        if (sn >= 0)
        {
          ijk[2] = rsOrder - ii - jj;
          int tOff = vtkLagrangeTriangle::Index(ijk, rsOrder);
          derivs[sn] = ll[kk] * td[tOff];
          derivs[sn + numPts] = ll[kk] * td[tOff];
