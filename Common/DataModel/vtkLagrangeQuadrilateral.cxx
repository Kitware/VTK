...
int vtkLagrangeQuadrilateral::EvaluatePosition(const double x[3], double closestPoint[3],
...
void vtkLagrangeQuadrilateral::Contour(double value, vtkDataArray* cellScalars,
...
void vtkLagrangeQuadrilateral::Clip(double value, vtkDataArray* cellScalars,
...
int vtkLagrangeQuadrilateral::Triangulate(int vtkNotUsed(index), vtkIdList* ptIds, vtkPoints* pts)
{
  ptIds->Reset();
  pts->Reset();
...
void vtkLagrangeQuadrilateral::Derivatives(int vtkNotUsed(subId),
...
const int* vtkLagrangeQuadrilateral::GetOrder()
{
  ...
  {
    int pointsPerAxis =
      static_cast<int>(ceil(pow(npts, 1. / 2.))); // number of points along each axis
    for (int i = 0; i < 2; ++i)
    {
 ...
 */
void vtkLagrangeQuadrilateral::PrepareApproxData(
...
/**\brief Given (i,j,k) coordinates within the Lagrange quad, return an offset into the local
