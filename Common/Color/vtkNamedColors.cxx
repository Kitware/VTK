...
public:
  ...
  void Init()
  {
    // Name and RGBA components for a named color, known entirely at compile time
    // so no runtime string parsing is needed to build the table below.
    ...
    //       earlier names.
    static constexpr NamedColorEntry colorTable[] = {
      // Name, R, G, B, A
    ...
    for (const NamedColorEntry& entry : colorTable)
    {
      this->colorMap[this->ToLowercase(entry.name)] =
        vtkColor4ub(entry.r, entry.g, entry.b, entry.a);
    }
  }
