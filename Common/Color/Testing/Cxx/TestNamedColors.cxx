...
bool TestEmptyColorName()
{
  ...
  {
    vtkGenericWarningMacro(<< "Fail: an empty color name "
...
bool TestNoSuchColor(vtkStdString const& name)
{
  ...
  {
    vtkGenericWarningMacro(<< "Fail: the color " << name << " exists when it shouldn't.");
    return false;
  }
  return true;
}
...
bool TestUnsignedChar(vtkStdString const& name)
{
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: arrays are not the same "
                           << "for color: " << name);
  }
  sameElements = true;
  nc->GetColor(name, v);
  ...
  {
    vtkGenericWarningMacro(<< "Fail: One of red, green blue or alpha do not match the array "
                           << "for color: " << name);
    return false;
  }
  return true;
...
bool TestDoubleRGB(vtkStdString const& name)
{
  ...
  {
    vtkGenericWarningMacro(<< "Fail: One of red, green or blue do not match the array "
                           << "for color: " << name);
    return false;
  }
  return true;
  ...
  const vtkStdString& name, const double dcolor[4], const unsigned char ucolor[4])
{
  ...
  {
    vtkGenericWarningMacro(<< "Fail: Setting a double color with an empty name.");
    nc->ResetColors();
    return false;
  }
  nc->SetColor("", ucolor);
  ...
  if (sz != nc->GetNumberOfColors())
  {
    vtkGenericWarningMacro(<< "Fail: Setting an unsigned char color with an empty name.");
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as double get as unsigned char, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as unsigned char get as double, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as unsigned char array get as vtkColor4ub, "
                           << "colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as vtkColor4ub array get as vtkColor4ub, "
                           << "colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as vtkColor3ub array get as vtkColor4ub, "
                           << "colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as unsigned char values get as vtkColor4ub, "
                           << "colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as double array get as vtkColor4d, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as vtkColor4d get as vtkColor4d, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as double values get as vtkColor4d, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  if (!sameElements)
  {
    vtkGenericWarningMacro(<< "Fail: Set as vtkColor3d get as vtkColor4d, colors do not match "
                           << "for color: " << name);
    nc->ResetColors();
    return false;
  }
  ...
  {
    vtkGenericWarningMacro(<< "Fail: Incorrect number of colors found, expected "
...
bool TestHTMLColorToRGBA()
{
  ...
  while (inputString[0] != '\n')
  {
    ...
    {
      vtkGenericWarningMacro(<< "Fail: input `" << inputString << "`"
                             << ", found " << outputColor << ", expected "
                             << vtkColor4ub(expectedOutput) << " instead.");
      testResult &= false;
    }
    ++i;
  }
...
int TestNamedColors(int vtkNotUsed(argc), char* vtkNotUsed(argv)[])
{
  ...
  if (!testResult)
  {
    vtkGenericWarningMacro(<< "Fail: TestNoSuchColor()");
  }
  ...
  for (std::vector<vtkStdString>::const_iterator p = cn.begin(); p != cn.end(); ++p)
  {
    ...
    {
      vtkGenericWarningMacro(<< "Fail: TestUnsignedChar(), with color " << *p);
      testResult &= false;
    }

    if (!TestDouble(*p))
    {
      vtkGenericWarningMacro(<< "Fail: TestDouble(), with color " << *p);
      testResult &= false;
    }

    if (!TestDoubleRGB(*p))
    {
      vtkGenericWarningMacro(<< "Fail: TestDoubleRGB(), with color " << *p);
      testResult &= false;
    }

    if (!TestUCharToDouble(*p))
    {
      vtkGenericWarningMacro(<< "Fail: TestUCharToDouble(), with color " << *p);
      testResult &= false;
    }
  }
  ...
  {
    vtkGenericWarningMacro(<< "Fail: TestAddingAColor(), with color " << name);
    testResult &= false;
  }

  if (!TestSearchForSynonyms())
  {
    vtkGenericWarningMacro(<< "Fail: TestSearchForSynonyms() - incorrect number of synonyms found, "
  ...
  {
    vtkGenericWarningMacro(<< "Fail: Incorrect number of colors"
  ...
  {
    vtkGenericWarningMacro(<< "Fail: GetNumberOfColors(), incorrect number of colors"
                           << "found " << cn.size() << ", expected " << NUMBER_OF_COLORS
                           << " instead.");
    testResult &= false;
  }

  vtkSmartPointer<vtkStringArray> vs = vtkSmartPointer<vtkStringArray>::New();
  nc->GetColorNames(vs);
  if (vs->GetNumberOfValues() != NUMBER_OF_COLORS)
  {
    vtkGenericWarningMacro(<< "Fail: GetColorNames(), incorrect number of colors"
  ...
  {
    vtkGenericWarningMacro(<< "Fail: PrintSelf() - a string of size " << PRINT_SELF_STRING_SIZE
  ...
  if (!testResult)
  {
    vtkGenericWarningMacro(<< "Fail: TestHTMLColorToRGBA()");
  }

  testResult &= TestRGBAToHTMLColor();
  if (!testResult)
  {
    vtkGenericWarningMacro(<< "Fail: TestRGBAToHTMLColor()");
  }
