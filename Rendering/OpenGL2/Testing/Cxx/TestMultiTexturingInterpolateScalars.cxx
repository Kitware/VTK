...
//------------------------------------------------------------------------------
int TestMultiTexturingInterpolateScalars(int argc, char* argv[])
{
  ...
  vtkPolyData* pd = vtkPolyData::SafeDownCast(elevationFilter->GetOutput());

  vtkFloatArray* tcoord = vtkFloatArray::SafeDownCast(pd->GetPointData()->GetTCoords());
  tcoord->SetName("TexCoords"); // Give texture coordinate a custom name for multitexturing
  ...
  actor->GetProperty()->SetTexture("skyclouds", tex);

  mapper->MapDataArrayToMultiTextureAttribute(
    "skyclouds", tcoord->GetName(), vtkDataObject::FIELD_ASSOCIATION_POINTS);
