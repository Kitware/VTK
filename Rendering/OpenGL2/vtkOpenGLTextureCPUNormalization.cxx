...
namespace
{
// The 32 bit float internal formats are not consecutive, so look them up.
GLenum NormalizedFloatInternalFormat(int numComps)
{
  switch (numComps)
  {
    case 1:
...
GLenum NormalizedFloatFormat(int numComps)
{
  switch (numComps)
  {
    case 1:
  ...
  unsigned int height)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, NormalizedFloatInternalFormat(numComps),
    static_cast<GLsizei>(width), static_cast<GLsizei>(height), 0, NormalizedFloatFormat(numComps),
    GL_FLOAT, dst);
