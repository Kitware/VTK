...
void main()
{
  ...
  if (cameraParallel != 0)
  {
    EyePos = vec3(vertexVC.x, vertexVC.y, vertexVC.z + radius);
  ...
  else
  {
    ...
    if (lengthED > radius)
    {
      EyePos = vertexVC.xyz - EyeDir*radius;
    }
  }
  ...
  vertexVC.xyz = normalVCVSOutput*radius + centerVCVSOutput;
