...
//------------------------------------------------------------------------------
void vtkOpenGLBatchedPolyDataMapper::RenderPieceStart(vtkRenderer* renderer, vtkActor* actor)
{
  ...
  for (auto& [polyData, element] : this->VTKPolyDataToGLBatchElement)
  {
  ...
  vtkShaderProgram* prog, GLBatchElement* glBatchElement, size_t primOffset)
{
  ...
  {
    prog->SetUniformi("blocktexture", batchElement.Texture->GetTextureUnit());
  }
}
...
//------------------------------------------------------------------------------
std::vector<vtkOpenGLPolyDataMapper::texinfo> vtkOpenGLBatchedPolyDataMapper::GetTextures(
  vtkActor* actor)
{
  std::vector<texinfo> textures = this->Superclass::GetTextures(actor);
