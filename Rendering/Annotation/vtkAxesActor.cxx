...
void vtkAxesActor::SetShaftType(int type)
{
  ...
  {
    if (type < vtkAxesActor::CYLINDER_SHAFT || type > vtkAxesActor::USER_DEFINED_SHAFT)
    {
      vtkErrorMacro("Undefined axes shaft type.");
      return;
    }

    if (type == vtkAxesActor::USER_DEFINED_SHAFT && this->UserDefinedShaft == nullptr)
    {
...
void vtkAxesActor::SetTipType(int type)
{
  ...
  {
    if (type < vtkAxesActor::CONE_TIP || type > vtkAxesActor::USER_DEFINED_TIP)
    {
      vtkErrorMacro("Undefined axes tip type.");
      return;
    }

    if (type == vtkAxesActor::USER_DEFINED_TIP && this->UserDefinedTip == nullptr)
    {
