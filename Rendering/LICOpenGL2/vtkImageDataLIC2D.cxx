...
int vtkImageDataLIC2D::SetContext(vtkRenderWindow* renWin)
{
  ...
  if (context)
  {
    ...
    bool featureSupport = vtkLineIntegralConvolution2D::IsSupported(context) &&
      vtkPixelBufferObject::IsSupported(context) &&
      vtkOpenGLFramebufferObject::IsSupported(context) && vtkRenderbuffer::IsSupported(context) &&
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (noiseInfo)
  {
    ...
    if (!noise)
    {
      vtkErrorMacro("Invalid noise dataset on input "
                    "Default noise dataset is used");
    }

    if ((noise->GetPointData() == nullptr) || (noise->GetPointData()->GetScalars() == nullptr))
    {
      vtkErrorMacro("Noise dataset missing point data scalars. "
                    "Default noise dataset is used");
      noise = nullptr;
    }
...
#if (vtkImageDataLIC2DDEBUG >= 1)
  vtkTextureIO::Write("idlic2d_vectors.vtk", vectorTex, nullptr, nullptr);
#endif
  ...
  if (this->Magnification > 1)
  {
    ...
    shaderHelper.Program = this->Context->GetShaderCache()->ReadyShaderProgram(vtkTextureObjectVS,
      "//VTK::System::Dec\n"
      "in vec2 tcoordVC;\n"
...
#if (vtkImageDataLIC2DDEBUG >= 1)
  vtkTextureIO::Write("idlic2d_magvectors.vtk", magVectorTex, nullptr, nullptr);
#endif
...
#if (vtkImageDataLIC2DDEBUG >= 1)
  vtkTextureIO::Write("idlic2d_noise.vtk", noiseTex, nullptr, nullptr);
#endif
  ...
  vtkPixelExtent magLicExtent(magUpdateExt[2 * comp[0]], magUpdateExt[2 * comp[0] + 1],
  ...
  vtkTextureObject* licTex = LICer->Execute(
...
#if (vtkImageDataLIC2DDEBUG >= 1)
  vtkTextureIO::Write("idlic2d_lic.vtk", licTex, nullptr, nullptr);
#endif
