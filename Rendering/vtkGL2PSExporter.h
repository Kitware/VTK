...
// are similar to the ones that GL2PS provides.  SimpleLineOffset,
// Silent, BestRoot, PS3Shading and OcclusionCull are similar to the
// options provided by GL2PS.  Please read the function documentation
// or the GL2PS documentation for more details.  The ivar
...
// configure VTK to compile this class and also to find the header
// file (gl2ps.h) and the gl2ps library.  Look for the VTK_USE_GL2PS
// advanced option when configuring VTK.  With gcc you can generate a
// libgl2ps.so like so:
//
// $ gcc -fPIC -g -O2 -c gl2ps.c; gcc -shared -o libgl2ps.so gl2ps.o
//
// or if you prefer static libraries:
//
...
//ETX
  ...
  // PS_FILE, EPS_FILE, TEX_FILE.  Defaults to EPS_FILE.  Depending on
  // the option chosen it generates the appropriate file (with correct
  // extension) when the Write function is called.
