...
vtkViewportSpecification<int> convert(
  const vtkViewportSpecification<double>& other, int width, int height)
{
  int value[4];
...
int vtkContextActor::RenderOverlay(vtkViewport* viewport)
{
  ...
  vtkViewportSpecification<double> view_viewport(viewport->GetViewport());
  ...
  vtkViewportSpecification<double> tile_viewport(viewport->GetVTKWindow()->GetTileViewport());
