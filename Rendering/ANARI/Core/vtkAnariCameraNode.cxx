...
vtkAnariCameraNode::~vtkAnariCameraNode()
{
  ...
  {
    anari::release(this->Internals->AnariDevice->GetHandle(), this->Internals->AnariCamera);
  }
...
void vtkAnariCameraNode::UpdateAnariObjectHandles()
{
  if (!this->Internals->AnariDevice)
  {
    this->Internals->AnariDevice = this->Internals->RendererNode->GetDevice();
  }
  ...
  if (!this->Internals->AnariCamera || parallel != this->Internals->IsParallelProjection)
  {
    ...
    this->Internals->AnariCamera = anari::newObject<anari::Camera>(
      this->Internals->AnariDevice->GetHandle(), parallel ? "orthographic" : "perspective");
...
void vtkAnariCameraNode::UpdateAnariCameraParameters()
{
  ...
  else
  {
    ...
    anari::setParameter(
      this->Internals->AnariDevice->GetHandle(), this->Internals->AnariCamera, "fovy", fovyRadians);
  }
  ...
  anari::setParameter(this->Internals->AnariDevice->GetHandle(), this->Internals->AnariCamera,
    "near", static_cast<float>(clippingRange[0]));
  anari::setParameter(this->Internals->AnariDevice->GetHandle(), this->Internals->AnariCamera,
    "far", static_cast<float>(clippingRange[1]));
  ...
  anari::setParameter(
    this->Internals->AnariDevice->GetHandle(), this->Internals->AnariCamera, "up", cameraUp);
