...
//-------------------------------------------------------------------------------------------------
- (vtkCocoaHardwareWindow*)getHardwareWindow
{
  return _myHardwareWindow;
}

//-------------------------------------------------------------------------------------------------
- (void)setHardwareWindow:(vtkCocoaHardwareWindow*)theHardwareWindow
{
  _myHardwareWindow = theHardwareWindow;
}
...
- (vtkCocoaRenderWindowInteractor*)getInteractor
{
  ...
  {
    return (vtkCocoaRenderWindowInteractor*)_myHardwareWindow->GetInteractor();
  }
  else
  {
    return nullptr;
...
- (void)invokeVTKKeyEvent:(unsigned long)theEventId cocoaEvent:(NSEvent*)theEvent
{
  ...
  vtkCocoaHardwareWindow* renWin = vtkCocoaHardwareWindow::SafeDownCast([self getHardwareWindow]);
  ...
  _keyRepeatCount = (isPress && type != NSEventTypeFlagsChanged && [theEvent isARepeat])
    ? (_keyRepeatCount + 1)
    : 0;
