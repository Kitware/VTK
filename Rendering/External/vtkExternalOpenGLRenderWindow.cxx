...
vtkglX::GLXFBConfig* vtkExtOpenGLRenderWindowTryForFBConfig(Display *DisplayId,
                                                          int drawable_type,
                                                          int doublebuff,
                                                          int stereo,
                                                          int multisamples,
...
XVisualInfo *vtkExtOpenGLRenderWindowTryForVisual(Display *DisplayId,
...
vtkglX::GLXFBConfig *vtkExtOpenGLRenderWindowGetDesiredFBConfig(
  Display *DisplayId,
  int &win_stereo,
  ...
  int &stencil)
{
  ...
  for (stereo = win_stereo; !fbc && stereo >= 0; stereo--)
    ...
    for (multi = win_multisamples; !fbc && multi >= 0; multi--)
      ...
      fbc = vtkExtOpenGLRenderWindowTryForFBConfig(DisplayId,
                                                 drawable_type,
                                                 win_doublebuffer,
                                                 stereo, multi,
                                                 win_alphaplanes,
...
XVisualInfo *vtkExternalOpenGLRenderWindow::GetDesiredVisualInfo()
{
  return 0;
    ...
    for (alpha = this->AlphaBitPlanes; !v && alpha >= 0; alpha--)
      ...
      for (stereo = this->StereoCapableWindow; !v && stereo >= 0; stereo--)
        ...
        for (multi = this->MultiSamples; !v && multi >= 0; multi--)
          ...
          v = vtkExtOpenGLRenderWindowTryForVisual(this->DisplayId,
                                                 this->DoubleBuffer,
                                                 stereo, multi, alpha,
                                                 stencil);
          if (v)
...
void vtkExternalOpenGLRenderWindow::Frame()
{
  return;
...
bool vtkExternalOpenGLRenderWindow::InitializeFromCurrentContext()
{
...
//
void vtkExternalOpenGLRenderWindow::SetStereoCapableWindow(int capable)
{
   return;
...
void vtkExternalOpenGLRenderWindow::CreateAWindow()
{
  return;
...
void vtkExternalOpenGLRenderWindow::DestroyWindow()
{
  return;
...
void vtkExternalOpenGLRenderWindow::CreateOffScreenWindow(int width, int height)
{
  return;
      ...
      if(v1 > 1 || (v1 == 1 && v2 >= 3))
        ...
        if(!this->Internal->PbufferContextId)
          ...
          if(loaded)
            {
            // get FBConfig
            vtkglX::GLXFBConfig* fb = vtkExtOpenGLRenderWindowGetDesiredFBConfig(
...
void vtkExternalOpenGLRenderWindow::DestroyOffScreenWindow()
{
  return;
...
void vtkExternalOpenGLRenderWindow::ResizeOffScreenWindow(int width, int height)
{
  return;
...
void vtkExternalOpenGLRenderWindow::WindowInitialize (void)
{
  return;
...
void vtkExternalOpenGLRenderWindow::Initialize (void)
{
    return;
...
void vtkExternalOpenGLRenderWindow::Finalize (void)
{
  return;
...
void vtkExternalOpenGLRenderWindow::SetFullScreen(int arg)
{
    return;
  int *temp;
...
void vtkExternalOpenGLRenderWindow::PrefFullScreen()
{
  return;
...
// Resize the window.
void vtkExternalOpenGLRenderWindow::WindowRemap()
{
  return;
...
void vtkExternalOpenGLRenderWindow::SetSize(int width,int height)
{
//  return;
...
int vtkExternalOpenGLRenderWindow::GetDesiredDepth()
{
  XVisualInfo *v;
  int depth = 0;
...
Visual *vtkExternalOpenGLRenderWindow::GetDesiredVisual ()
{
  return 0;
...
Colormap vtkExternalOpenGLRenderWindow::GetDesiredColormap ()
{
  XVisualInfo *v;
...
void vtkExternalOpenGLRenderWindow::MakeCurrent()
{
...
void vtkExternalOpenGLRenderWindow::SetForceMakeCurrent()
{
  this->ForceMakeCurrent = 1;
}

int vtkExtOpenGLRenderWindowFoundMatch;

extern "C"
{
  Bool vtkExtOpenGLRenderWindowPredProc(Display *vtkNotUsed(disp),
                                      XEvent *event,
                                      char *arg)
  {
    ...
    if (((reinterpret_cast<XAnyEvent *>(event))->window == win) &&
      ...
      {
      vtkExtOpenGLRenderWindowFoundMatch = 1;
      }
...
void *vtkExternalOpenGLRenderWindow::GetGenericContext()
{
...
int vtkExternalOpenGLRenderWindow::GetEventPending()
{
  XEvent report;

  vtkExtOpenGLRenderWindowFoundMatch = 0;
  if (this->OffScreenRendering)
    {
    return vtkExtOpenGLRenderWindowFoundMatch;
    }
  XCheckIfEvent(this->DisplayId, &report, vtkExtOpenGLRenderWindowPredProc,
                reinterpret_cast<char *>(this->WindowId));
  return vtkExtOpenGLRenderWindowFoundMatch;
}

// Get the size of the screen in pixels
int *vtkExternalOpenGLRenderWindow::GetScreenSize()
{
  // get the default display connection
  if (!this->DisplayId)
    {
...
int *vtkExternalOpenGLRenderWindow::GetPosition(void)
{
  XWindowAttributes attribs;
  int x,y;
  Window child;
...
Display *vtkExternalOpenGLRenderWindow::GetDisplayId()
{
...
Window vtkExternalOpenGLRenderWindow::GetParentId()
{
...
Window vtkExternalOpenGLRenderWindow::GetWindowId()
{
...
void vtkExternalOpenGLRenderWindow::SetPosition(int x, int y)
{
...
void vtkExternalOpenGLRenderWindow::SetParentId(Window arg)
{
//   if (this->ParentId)
//     {
...
void vtkExternalOpenGLRenderWindow::SetWindowId(Window arg)
{
...
// Set this RenderWindow's X window id to a pre-existing window.
void vtkExternalOpenGLRenderWindow::SetWindowInfo(char *info)
{
  int tmp;
...
// Set this RenderWindow's X window id to a pre-existing window.
void vtkExternalOpenGLRenderWindow::SetNextWindowInfo(char *info)
{
  int tmp;
  sscanf(info,"%i",&tmp);
...
void vtkExternalOpenGLRenderWindow::SetParentInfo(char *info)
{
  int tmp;
...
void vtkExternalOpenGLRenderWindow::SetWindowId(void *arg)
{
  this->SetWindowId(reinterpret_cast<Window>(arg));
}
void vtkExternalOpenGLRenderWindow::SetParentId(void *arg)
{
  this->SetParentId(reinterpret_cast<Window>(arg));
}

const char* vtkExternalOpenGLRenderWindow::ReportCapabilities()
{
  MakeCurrent();
...
int vtkExternalOpenGLRenderWindow::SupportsOpenGL()
{
  this->MakeCurrent();
  if (!this->DisplayId)
    {
...
int vtkExternalOpenGLRenderWindow::IsDirect()
{
  this->MakeCurrent();
  this->UsingHardware = 0;
  if (this->OffScreenRendering && this->Internal->PbufferContextId)
...
void vtkExternalOpenGLRenderWindow::SetWindowName(const char * cname)
{
...
void vtkExternalOpenGLRenderWindow::SetNextWindowId(Window arg)
{
...
void vtkExternalOpenGLRenderWindow::SetNextWindowId(void *arg)
{
...
// X display id.
void vtkExternalOpenGLRenderWindow::SetDisplayId(Display  *arg)
{
...
void vtkExternalOpenGLRenderWindow::SetDisplayId(void *arg)
{
...
//----------------------------------------------------------------------------
void vtkExternalOpenGLRenderWindow::HideCursor()
{
...
//----------------------------------------------------------------------------
void vtkExternalOpenGLRenderWindow::ShowCursor()
{
  if (!this->DisplayId || !this->WindowId)
    {
    this->CursorHidden = 0;
...
//============================================================================
// Stuff above this is almost a mirror of vtkExternalOpenGLRenderWindow.
...
// (mangled) from vtkExternalOpenGLRenderWindow like the other OpenGL classes.
//============================================================================

void vtkExternalOpenGLRenderWindow::SetOffScreenRendering(int i)
{
...
void *vtkExternalOpenGLRenderWindow::GetGenericWindowId()
{
#ifdef VTK_OPENGL_HAS_OSMESA
...
void vtkExternalOpenGLRenderWindow::SetCurrentCursor(int shape)
{
