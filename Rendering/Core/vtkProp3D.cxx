...
void vtkProp3D::SetOrientation(double x, double y, double z)
{
  ...
  if (x == this->Orientation[0] && y == this->Orientation[1] && z == this->Orientation[2])
  {
    return;
  }
  this->IsIdentity = 0;
  ...
  vtkDebugMacro(<< " Orientation set to ( " << this->Orientation[0] << ", " << this->Orientation[1]
                << ", " << this->Orientation[2] << ")\n");
...
void vtkProp3D::GetOrientation(double o[3])
{
  ...
  vtkDebugMacro(<< " Returning Orientation of ( " << o[0] << ", " << o[1] << ", " << o[2] << ")\n");
}
...
void vtkProp3D::AddOrientation(double a1, double a2, double a3)
{
  ...
  this->SetOrientation(orient[0] + a1, orient[1] + a2, orient[2] + a3);
}
...
void vtkProp3D::PokeMatrix(vtkMatrix4x4* matrix)
{
  ...
  if (matrix != nullptr) // set a new transformation
  {
    ...
    if (this->UserTransform && this->UserTransform->GetMatrix() == this->UserMatrix)
    {
  ...
  else // we restore our original state
  {
    if (this->CachedProp3D != nullptr)
    {
      ...
      if (this->CachedProp3D->UserTransform &&
        this->CachedProp3D->UserTransform->GetMatrix() == this->CachedProp3D->UserMatrix)
      {
