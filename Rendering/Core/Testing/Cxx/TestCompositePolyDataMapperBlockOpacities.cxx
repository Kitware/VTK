...
int TestCompositePolyDataMapperBlockOpacities(int argc, char* argv[])
{
  ...
  // Make the actor transparent and test if the per-block opacity override value is correctly
  // applied.
    ...
    [](vtkObject* caller, unsigned long, void* clientdata, void*)
    {
            ...
            vtkPartitionedDataSetCollection::SafeDownCast(inputMapper->GetInputDataObject(0, 0)))
      {
        ...
        if (auto* mesh = compositeMesh->GetPartitionAsDataObject(selectedSphere, 0))
        {
          ...
          std::cout << "Make sphere at flat index: " << flatIndex << " opaque\n";
          inputCda->RemoveBlockOpacities();
          inputCda->SetBlockOpacity(mesh, 1.0);
  ...
  bool interactiveTestFailed = false;
  for (int i = 0; i < 5; ++i)
  {
    ...
    if (testing->RegressionTest(0.01, cout) == vtkTesting::FAILED)
    {
      ...
      // write the valid/difference image.
      interactiveTestFailed |= true;
    }
  }
  return interactiveTestFailed ? EXIT_FAILURE : EXIT_SUCCESS;
}
