...
float* vtkVolume::GetScalarOpacityArray(int index)
{
  ...
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP << "]: " << index);
    return nullptr;
  }
...
double vtkVolume::ComputeScreenCoverage(vtkViewport* vp)
{
  ...
  if (ren)
  {
    ...
    vtkMatrix4x4* mat = cam->GetCompositeProjectionTransformMatrix(aspect[0] / aspect[1], 0.0, 1.0);
...
void vtkVolume::UpdateScalarOpacityforSampleSize(
  vtkRenderer* vtkNotUsed(ren), float sample_distance)
{
  int i;
