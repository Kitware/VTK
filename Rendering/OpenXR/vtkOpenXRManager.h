...
public:
  ...
  // Todo a flag to invalidate / validate pose at each frame (at beginframe)

  /**
   * Returns the view pose that contains the view orientation and position for the specified
   * eye.
   */
  const XrPosef& GetViewPose(const uint32_t eye)
  {
    return this->RenderResources->ProjectionLayerViews[eye].pose;
  }

  /**
   * Returns the projection field of view for the specified eye.
   */
  const XrFovf& GetProjectionFov(const uint32_t eye)
  {
   ...
   */
  bool PrepareRendering(const uint32_t eye, GLuint& colorTextureId, GLuint& depthTextureId);

  /**
   * When the rendering in a swapchain image is done, it must be released with
   * this function.
   */
  void ReleaseSwapchainImage(const uint32_t eye);
...
protected:
  ...
  // HeadMountedDisplay (HMD) or Android
  ...
  // Three available types: VIEW, LOCAL and STAGE
  // The VIEW space tracks the view origin
  // The LOCAL space establishes a world-locked origin
  constexpr static XrReferenceSpaceType ReferenceSpaceType = XR_REFERENCE_SPACE_TYPE_LOCAL;

  // Communication with the runtime through the instance
  XrInstance Instance;
