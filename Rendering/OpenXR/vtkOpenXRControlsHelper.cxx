...
vtkOpenXRControlsHelper::vtkOpenXRControlsHelper()
{
  ...
  this->DrawSide = vtkOpenXRControlsHelper::Right;
  this->ButtonSide = vtkOpenXRControlsHelper::Back;
  ...
  this->MoveCallbackCommand->SetCallback(vtkOpenXRControlsHelper::MoveEvent);
...
void vtkOpenXRControlsHelper::SetDevice(vtkEventDataDevice val)
{
  if (this->Device == val)
  {
    return;
...
{
  vtkOpenXRRenderWindowInteractor* iren = static_cast<vtkOpenXRRenderWindowInteractor*>(
  ...
  vtkOpenXRRenderWindow* renWin = static_cast<vtkOpenXRRenderWindow*>(iren->GetRenderWindow());
  if (!renWin)
  {
    return;
  }
  ...
  // Warning: for instance we have no openxr models
  // we have just a ray
  const int trackedDeviceIndex = renWin->GetTrackedDeviceIndexForDevice(controller);
  vtkOpenXRRay* ray = renWin->GetTrackedDeviceModel(trackedDeviceIndex);
  ...
  uint32_t nbOfComponents =
    renWin->GetOpenVRRenderModels()->GetComponentCount(ray->GetName().c_str());
...
void vtkOpenXRControlsHelper::MoveEvent(vtkObject*, unsigned long, void* clientdata, void* calldata)
{
  vtkOpenXRControlsHelper* self = static_cast<vtkOpenXRControlsHelper*>(clientdata);

  vtkOpenXRRenderWindow* renWin =
    static_cast<vtkOpenXRRenderWindow*>(self->Renderer->GetRenderWindow());
...
//------------------------------------------------------------------------------
void vtkOpenXRControlsHelper::UpdateRepresentation()
{
  this->NeedUpdate = false;
  if (!this->Enabled)
  {
  ...
  {
    vtkOpenXRRenderWindow* renWin =
      static_cast<vtkOpenXRRenderWindow*>(this->Renderer->GetRenderWindow());
    if (!renWin)
    {
      return;
    }
    ...
    vtkOpenXRRenderWindowInteractor* iren =
      static_cast<vtkOpenXRRenderWindowInteractor*>(renWin->GetInteractor());
    if (!iren)
    {
      return;
    }
...
//------------------------------------------------------------------------------
void vtkOpenXRControlsHelper::ReleaseGraphicsResources(vtkWindow* w)
{
...
//------------------------------------------------------------------------------
int vtkOpenXRControlsHelper::RenderOpaqueGeometry(vtkViewport* v)
{
  if (this->NeedUpdate)
  {
...
//------------------------------------------------------------------------------
int vtkOpenXRControlsHelper::RenderTranslucentPolygonalGeometry(vtkViewport* v)
{
  if (this->NeedUpdate)
  {
    this->UpdateRepresentation();
...
//------------------------------------------------------------------------------
vtkTypeBool vtkOpenXRControlsHelper::HasTranslucentPolygonalGeometry()
{
...
//------------------------------------------------------------------------------
void vtkOpenXRControlsHelper::BuildRepresentation()
{
...
//------------------------------------------------------------------------------
void vtkOpenXRControlsHelper::SetText(vtkStdString _text)
{
  if (this->Text == _text)
  {
    return;
...
void vtkOpenXRControlsHelper::SetEnabled(bool val)
{
  if (val == this->Enabled)
  {
    return;
...
//------------------------------------------------------------------------------
void vtkOpenXRControlsHelper::SetRenderer(vtkRenderer* ren)
{
...
//------------------------------------------------------------------------------
vtkRenderer* vtkOpenXRControlsHelper::GetRenderer()
{
