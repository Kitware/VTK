...
"""Verify the GPU evaluation of the vtkDGWdg "G" (HGRAD) basis functions.
"""
...
class TestCellGridWdgGaussBasis(Testing.vtkTest):

    dataFile = os.path.join(VTK_DATA_ROOT, 'Data', 'dgWedges-GBasis.dg')
    ...
    def _readCoefficients(self):
        ...
        for name, _ in FIELDS:
            array = grid.GetAttributes('vtkDGWdg').GetArray(name)
    ...
    def testMontageRendering(self):
        ...
        window = rr.vtkRenderWindow(
            ...
            window_name="vtkDGWdg HGRAD 'G' basis",
        )
...
if __name__ == "__main__":
    Testing.main([(TestCellGridWdgGaussBasis, 'test')])
