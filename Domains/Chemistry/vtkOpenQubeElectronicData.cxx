...
vtkImageData* vtkOpenQubeElectronicData::CalculateMO(vtkIdType orbitalNumber)
{
  ...
  {
    vtkWarningMacro(<< "Unable to calculateMO for orbital " << orbitalNumber << " in OpenQube.");
    return 0;
  }
  ...
  vtkDebugMacro(<< "Converting OpenQube::Cube to vtkImageData for MO " << orbitalNumber);

  // Copy cube --> image
  this->FillImageDataFromQube(cube, image);
  image->SetOrbitalNumber(orbitalNumber);

  vtkDebugMacro(<< "Adding vtkImageData to this->Images for MO " << orbitalNumber);
...
void vtkOpenQubeElectronicData::FillImageDataFromQube(OpenQube::Cube* qube, vtkImageData* image)
{
  ...
  {
    vtkWarningMacro(<< "Cannot cast input image to internal OQEDImageData type.");
  }
