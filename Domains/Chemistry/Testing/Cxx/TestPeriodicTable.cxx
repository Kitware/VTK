...
int TestPeriodicTable(int, char*[])
{
  ...
  {
    std::cout << "vtkPeriodicTable::GetAtomicNumber cannot parse numeric "
              << "strings properly. Given \"25\", should get 25, got "
              << pTab->GetAtomicNumber("25") << std::endl;
    ++errors;
  }
  if (pTab->GetAtomicNumber("300") != 0 || pTab->GetAtomicNumber("-300") != 0)
  {
    std::cout << "vtkPeriodicTable does not return 0 for invalid numeric strings. "
              << "Given \"300\" and \"-300\", returned " << pTab->GetAtomicNumber("300") << " and "
              << pTab->GetAtomicNumber("-300") << " respectively." << std::endl;
    ++errors;
  }
  ...
  {
    std::cout << "vtkPeriodicTable did not return 0 for an invalid string: "
              << pTab->GetAtomicNumber("I'm not an element.") << ", "
              << pTab->GetAtomicNumber(nullptr) << ", " << pTab->GetAtomicNumber(nullString)
              << std::endl;
    ++errors;
  }
  ...
  for (unsigned short i = 0; i <= pTab->GetNumberOfElements(); ++i)
  {
    ...
    {
      std::cout << "Element name failed roundtrip: Name: \"" << name << "\" atomic number: " << i
                << "vtkPeriodicTable::GetAtomicNumber(\"" << name
                << "\") returns: " << pTab->GetAtomicNumber(name) << std::endl;
      ++errors;
    }

    if (pTab->GetAtomicNumber(symbol) != i)
    {
      std::cout << "Element symbol failed roundtrip: Symbol: \"" << symbol
                << "\" atomic number: " << i << " vtkPeriodicTable::GetAtomicNumber(\"" << symbol
                << "\") returns: " << pTab->GetAtomicNumber(symbol) << std::endl;
      ++errors;
    }
  }
  ...
  {
    std::cout << "Failed to identify \"D\" as a hydrogen isotope. "
              << "Atomic number for \"D\": " << pTab->GetAtomicNumber("D") << std::endl;
    ++errors;
  }
  if (pTab->GetAtomicNumber("Deuterium") != pTab->GetAtomicNumber("Hydrogen"))
  {
    std::cout << "Failed to identify \"Deuterium\" as a hydrogen isotope. "
              << "Atomic number for \"Deuterium\": " << pTab->GetAtomicNumber("Deuterium")
              << std::endl;
    ++errors;
  }
  //  - Tritium
  ...
  {
    std::cout << "Failed to identify \"T\" as a hydrogen isotope. "
              << "Atomic number for \"T\": " << pTab->GetAtomicNumber("T") << std::endl;
    ++errors;
  }
  if (pTab->GetAtomicNumber("Tritium") != pTab->GetAtomicNumber("Hydrogen"))
  {
    std::cout << "Failed to identify \"Tritium\" as a hydrogen isotope. "
              << "Atomic number for \"Tritium\": " << pTab->GetAtomicNumber("Tritium") << std::endl;
    ++errors;
  }
  //  - Aluminum, Aluminium
  if (pTab->GetAtomicNumber("Aluminum") != pTab->GetAtomicNumber("Aluminium"))
  {
    std::cout << "\"Aluminum\" returns a different atomic number than \"Aluminium\", "
              << "(" << pTab->GetAtomicNumber("Aluminum") << " and "
              << pTab->GetAtomicNumber("Aluminium") << " respectively)." << std::endl;
    ++errors;
  }
  ...
  {
    std::cout << "Failed to obtain the correct atomic number for " << symbolString << ": "
              << pTab->GetAtomicNumber(symbolString) << std::endl;
    ++errors;
  }
  ...
  {
    ++errors;
    std::cout << "Expected color for carbon was incorrect: " << color[0] << ", " << color[1] << ", "
              << color[2] << std::endl;
  }
  ...
  {
    std::cout << "Expected color for oxygen was incorrect: " << rgb[0] << ", " << rgb[1] << ", "
              << rgb[2] << std::endl;
    ++errors;
  }
  ...
  {
    std::cout << "Incorrect radius: " << setprecision(8) << radius << std::endl;
    ++errors;
  }
  ...
  {
    std::cout << "Error, lookup table has " << lookupTable->GetNumberOfColors()
              << " colors, expected 119." << std::endl;
    ++errors;
  }
