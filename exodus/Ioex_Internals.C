...
int Internals::put_metadata(const Mesh &mesh, const CommunicationMetaData &comm)
{
  ...
  if (rootid == exodusFilePtr) {
    ...
    if (comm.outputNemesis && comm.processorCount > 1) {
      std::array<int, 2> ltempsv{comm.processorCount, comm.processorId};
  ...
  if (!mesh.nodeblocks.empty() && mesh.nodeblocks[0].entityCount > 0) {
    ...
    if (mesh.use_node_map) {
      std::array<int, 1> dims1{numnoddim};
  ...
  if (!mesh.nodeblocks.empty() && mesh.nodeblocks[0].attributeCount > 0) {
    ...
    std::array<int, 2> dims{numnoddim, numattrdim};
    status =
  ...
  if (elem_count > 0) {
    ...
    if (mesh.use_elem_map) {
      std::array<int, 1> dims{numelemdim};
      varid  = 0;
  ...
  if (face_count > 0) {
    ...
    if (mesh.use_face_map) {
      std::array<int, 1> dims{numfacedim};
      varid  = 0;
  ...
  if (edge_count > 0) {
    ...
    if (mesh.use_edge_map) {
      std::array<int, 1> dims{numedgedim};
      varid  = 0;
  ...
  if (comm.outputNemesis) {
    ...
    // by this quantity.
    {
      ...
      const std::array<int, 2>            types{ids_type, bulk_type};
    ...
    // by this quantity.
    {
      ...
      const std::array<int, 3>            types{ids_type, bulk_type, bulk_type};
...

    {
      std::array<const char *, 4> vars{VAR_N_COMM_IDS, VAR_N_COMM_STAT, VAR_N_COMM_DATA_IDX,
                                       nullptr};
      const std::array<int, 3>            types{ids_type, NC_INT, bulk_type};
    ...
    }
    {
      ...
      const std::array<int, 2>            types{ids_type, NC_INT};
...
int Internals::put_metadata(const std::vector<ElemBlock> &blocks, bool count_only)
{
  ...
  for (size_t iblk = 0; iblk < num_elem_blk; iblk++) {
    ...
    {
      std::array<int, 2> dims{numelbdim, nelnoddim};
      int        connid = 0;
    ...
    if (blocks[iblk].edgesPerEntity > 0) {
      ...
      std::array<int, 2> dims{numelbdim, neledgdim};
    ...
    if (blocks[iblk].facesPerEntity > 0) {
      ...
      std::array<int, 2> dims{numelbdim, nelfacdim};
    ...
    if (blocks[iblk].attributeCount > 0) {
      ...
      {
        std::array<int, 2> dims{numelbdim, numattrdim};
        int        varid = 0;
      ...
      {
        // Attribute name array...
        std::array<int, 2> dims{numattrdim, namestrdim};
        int        varid = 0;
        status =
...
int Internals::put_metadata(const std::vector<NodeSet> &nodesets, bool count_only)
{
  ...
  for (int i = 0; i < num_node_sets; i++) {
    ...
    if (nodesets[i].attributeCount > 0) {
      ...
      {
        std::array<int, 2> dims{dimid, numattrdim};
...
#pragma warning(pop)
                             ...
                             int dim_dim, int str_dim)
  {
    ...
    std::array<int, 2> dim{dim_dim, str_dim};
                         ...
                         const char *stat_var, const char *id_var, const char *name_var)
  {
    ...
    if (name_var != nullptr) {
      std::array<int, 2> dim{dimid, namestrdim};
