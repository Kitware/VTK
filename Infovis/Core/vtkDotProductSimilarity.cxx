...
static double DotProduct(vtkDenseArray<double>* input_a, vtkDenseArray<double>* input_b,
  ...
  const vtkIdType component_dimension, const vtkArrayRange range_a, const vtkArrayRange range_b)
{
  ...
  vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  try
  {
    ...
    vtkDenseArray<double>* const input_array_a =
      vtkDenseArray<double>::SafeDownCast(input_a->GetArray(static_cast<vtkIdType>(0)));
    if (!input_array_a)
    ...
    if (input_b)
    {
      ...
      input_array_b =
        vtkDenseArray<double>::SafeDownCast(input_b->GetArray(static_cast<vtkIdType>(0)));
      if (!input_array_b)
    ...
    const vtkArrayRange vectors_b =
      input_array_b ? input_array_b->GetExtent(vector_dimension) : vtkArrayRange();
    const vtkArrayRange components_b =
      input_array_b ? input_array_b->GetExtent(component_dimension) : vtkArrayRange();
    ...
    if (input_array_b)
    {
      ...
      if (this->FirstSecond)
      {
        for (vtkIdType vector_a = vectors_a.GetBegin(); vector_a != vectors_a.GetEnd(); ++vector_a)
        {
          ...
          for (vtkIdType vector_b = vectors_b.GetBegin(); vector_b != vectors_b.GetEnd();
               ++vector_b)
          {
            // Can't use std::make_pair - see
            // http://sahajtechstyle.blogspot.com/2007/11/whats-wrong-with-sun-studio-c.html
            similarities.insert(std::pair<const double, vtkIdType>(
      ...
      if (this->SecondFirst)
      {
        for (vtkIdType vector_b = vectors_b.GetBegin(); vector_b != vectors_b.GetEnd(); ++vector_b)
        {
          ...
          for (vtkIdType vector_a = vectors_a.GetBegin(); vector_a != vectors_a.GetEnd();
               ++vector_a)
          {
            // Can't use std::make_pair - see
            // http://sahajtechstyle.blogspot.com/2007/11/whats-wrong-with-sun-studio-c.html
