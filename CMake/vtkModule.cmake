...
function (vtk_module_scan)
  ...
  foreach (_vtk_scan_module_file IN LISTS _vtk_scan_MODULE_FILES)
    ...
    if (DEFINED _vtk_scan_REQUEST_MODULE)
      list(FIND _vtk_scan_REQUEST_MODULE "${_vtk_scan_module_name}" _vtk_scan_idx)
      if (_vtk_scan_idx EQUAL -1)
    ...
    if (DEFINED _vtk_scan_REJECT_MODULES)
      list(FIND _vtk_scan_REJECT_MODULES "${_vtk_scan_module_name}" _vtk_scan_idx)
    ...
    if (${_vtk_scan_module_name}_KIT)
      ...
      list(FIND _vtk_scan_all_kits "${${_vtk_scan_module_name}_KIT}" _vtk_scan_idx)
      if (_vtk_scan_idx EQUAL -1)
        message(FATAL_ERROR
  ...
  foreach (_vtk_scan_module IN LISTS _vtk_scan_all_modules)
    list(FIND _vtk_scan_current_modules "${_vtk_scan_module}" _vtk_scan_idx)
    if (_vtk_scan_idx EQUAL -1)
      continue ()
    endif ()
  ...
  foreach (_vtk_scan_module IN LISTS _vtk_scan_all_modules)
    ...
    if (_vtk_scan_provide_${_vtk_scan_module})
      ...
      foreach (_vtk_scan_module_depend IN LISTS "${_vtk_scan_module}_DEPENDS" "${_vtk_scan_module}_PRIVATE_DEPENDS" _vtk_scan_test_depends _vtk_scan_test_wants)
        ...
        list(FIND _vtk_scan_current_modules "${_vtk_scan_module_depend}" _vtk_scan_idx)
        if (_vtk_scan_idx EQUAL -1)
...
function (vtk_module_build)
  ...
  foreach (_vtk_build_test IN LISTS _vtk_build_test_modules)
    list(FIND _vtk_build_MODULES "${_vtk_build_test}" _vtk_build_test_idx)
    if (_vtk_build_test_idx EQUAL "-1")
      continue ()
    endif ()
...
function (vtk_module_autoinit)
  ...
  while (_vtk_autoinit_module_stack)
    ...
    list(FIND _vtk_autoinit_seen "${_vtk_autoinit_current_module}" _vtk_autoinit_idx)
    if (NOT _vtk_autoinit_idx EQUAL "-1")
      continue ()
    endif ()
...
while (_vtk_module_find_package_components_to_check)
  ...
  list(FIND _vtk_module_find_package_components_checked \"${_vtk_module_component}\" _vtk_component_idx)
  if (NOT _vtk_component_idx EQUAL \"-1\")
    continue ()
  endif ()
...
if (_vtk_module_find_package_components)
  list(FIND _vtk_module_find_package_components \"${_vtk_export_target_name}\" idx)
  if (NOT idx EQUAL -1)
    set(_vtk_module_find_package_enabled ON)
    list(FIND _vtk_module_find_package_components_required \"${_vtk_export_target_name}\" idx)
    if (NOT idx EQUAL -1)
