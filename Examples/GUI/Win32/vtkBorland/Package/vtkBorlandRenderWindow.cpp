...
void __fastcall TvtkBorlandRenderWindow::Paint(void)
{
    if (FRenderWindow)
    {
        ...
        catch (...)
        {
            ...
            // ShowErrorInfo("An exception occurred whilst rendering");
...
void __fastcall TvtkBorlandRenderWindow::WMChar(TWMKey &Message)
{
    switch (Message.CharCode)
    {
        ...
        case 'q':
        case 'Q':
            if (!FOnVtkClose || (FOnVtkClose && FOnVtkClose(this)))
            {
...
void __fastcall TvtkBorlandRenderWindow::MouseDown(TMouseButton button, TShiftState shift, int x, int y )
{
    ...
    if (FInteractor && FUsevtkInteractor)
    {
        ...
        {
          case mbLeft:
            FInteractor->OnLButtonDown(Handle, flags, x,y);
            break;
          case mbRight:
            FInteractor->OnRButtonDown(Handle, flags, x,y);
            break;
          case mbMiddle:
            FInteractor->OnMButtonDown(Handle, flags, x,y);
            break;
        }
    }
}
...
void __fastcall TvtkBorlandRenderWindow::MouseUp(TMouseButton button, TShiftState shift, int x, int y )
{
    ...
    if (FInteractor && FUsevtkInteractor)
    {
        ...
        {
          case mbLeft:
            FInteractor->OnLButtonUp(Handle, flags, x,y);
            break;
          case mbRight:
            FInteractor->OnRButtonUp(Handle, flags, x,y);
            break;
          case mbMiddle:
            FInteractor->OnMButtonUp(Handle, flags, x,y);
            break;
        }
    }
}
