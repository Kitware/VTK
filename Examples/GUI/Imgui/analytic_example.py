...
# Frustum selection picks voxels through the volume.
...
def custom_gui():
    ...
    # Rubber band overlay in selection mode.
    if state.interaction_mode == 1:
        io = imgui.get_io()
...
viewer.run(custom_gui=custom_gui, title="RTAnalytic Selection Demo")
