...
void vtkRTTestSequence::Run()
{
  ...
  this->Test->SetRenderSize(
    this->RenderTimings->GetRenderWidth(), this->RenderTimings->GetRenderHeight());
  ...
  while (remainingTime > 1.5 * lastRunTime &&
    (sequenceEnd == 0 || this->SequenceCount <= sequenceEnd) && lastRunTime < stepLimit)
  {
    ...
    if (this->ChartResults)
    {
      int numRows = this->SequenceCount - this->RenderTimings->GetSequenceStart() + 1;
      results->SetNumberOfRows(numRows);
      results->SetValue(numRows - 1, 0, tr.Results[this->Test->GetSecondSummaryResultName()]);
      results->SetValue(numRows - 1, 1, tr.Results[this->Test->GetSummaryResultName()]);
      results->Modified();
      if (numRows > 1)
      {
...
void vtkRTTestSequence::ReportSummaryResults(ostream& ost)
{
  ...
  ost << this->Test->GetName() << ":" << bestTestResult->SequenceNumber << ": " << result << " "
      << this->Test->GetSummaryResultName() << " and "
      << (static_cast<vtkIdType>(
            10.0 * bestTestResult->Results[this->Test->GetSecondSummaryResultName()]) /
           10.0)
...
int vtkRenderTimings::ParseCommandLineArguments(int argc, char* argv[])
{
  ...
  this->Arguments.AddArgument("-tls", argT::SPACE_ARGUMENT, &this->SequenceStepTimeLimit,
  ...
  this->Arguments.AddBooleanArgument(
  ...
  this->Arguments.AddBooleanArgument(
    "-list", &this->ListTests, "Provide a listing of available tests.");
  this->Arguments.AddBooleanArgument(
    "-nochart", &this->NoChartResults, "Suppress realtime charting of test performance.");
  this->Arguments.AddArgument(
    "-width", argT::SPACE_ARGUMENT, &this->RenderWidth, "Width of benchmark rendering window.");
  this->Arguments.AddArgument(
    "-height", argT::SPACE_ARGUMENT, &this->RenderHeight, "Height of benchmark rendering window.");
