...
vtkCxxRevisionMacro(vtkGenericStreamer, "1.6");
vtkCxxSetObjectMacro(vtkGenericStreamer,Integrator,vtkInitialValueProblemSolver);

#define VTK_START_FROM_POSITION 0
#define VTK_START_FROM_LOCATION 1

vtkGenericStreamer::StreamArray::StreamArray()
{
  this->MaxId = -1; 
  this->Array = new vtkGenericStreamer::StreamPoint[1000];
  this->Size = 1000;
  this->Extend = 5000;
  this->Direction = VTK_INTEGRATE_FORWARD;
}

vtkGenericStreamer::StreamPoint *vtkGenericStreamer::StreamArray::Resize(vtkIdType sz)
{
  vtkGenericStreamer::StreamPoint *newArray;
  vtkIdType newSize;
  ...
  newArray = new vtkGenericStreamer::StreamPoint[newSize];

  memcpy(newArray, this->Array,
         (sz < this->Size ? sz : this->Size) * sizeof(vtkGenericStreamer::StreamPoint));
...
void vtkGenericStreamer::SetSource(vtkDataSet *source)
{
  this->vtkProcessObject::SetNthInput(1, source);
}

vtkDataSet *vtkGenericStreamer::GetSource()
{
...
void vtkGenericStreamer::SetStartLocation(vtkIdType cellId, int subId,
                                   double pcoords[3])
{
...
// Specify the start of the streamline in the cell coordinate system. That is,
// cellId and subId (if composite cell), and parametric coordinates.
void vtkGenericStreamer::SetStartLocation(vtkIdType cellId, int subId, double r,
                                   double s, double t)
{
  double pcoords[3];
  pcoords[0] = r;
...
vtkIdType vtkGenericStreamer::GetStartLocation(int& subId, double pcoords[3])
{
...
void vtkGenericStreamer::SetStartPosition(double x[3])
{
...
// Specify the start of the streamline in the global coordinate system. Search
// must be performed to find initial cell to start integration from.
void vtkGenericStreamer::SetStartPosition(double x, double y, double z)
{
  double pos[3];
  pos[0] = x;
  pos[1] = y;
...
VTK_THREAD_RETURN_TYPE vtkGenericStreamer::ThreadedIntegrate( void *arg )
{
  vtkGenericStreamer *self;
  int                      thread_count;
  int                      thread_id;
  vtkGenericStreamer::StreamArray *streamer;
  vtkGenericStreamer::StreamPoint *sNext = 0, *sPtr;
  vtkGenericStreamer::StreamPoint pt1, pt2;
  int                      i;
...
//        input->GetCell(pt1.cellId, cell); //FIXME
...
//        input->GetCell(pt2.cellId, cell);//FIXME
...
//  if ( ! (inVectors=pd->GetVectors()) )//FIXME
//    {
//    delete [] w;
...
//  inScalars = pd->GetScalars();//FIXME
  ...
  this->Streamers = new vtkGenericStreamer::StreamArray[this->NumberOfStreamers];
...
//                                   sPtr->subId, sPtr->p, w);//FIXME
    ...
    sPtr = this->Streamers[0].GetStreamPoint(idx);
    sPtr->subId = 0;
//    cell =  input->GetCell(sPtr->cellId);//FIXME
//    cell->EvaluateLocation(sPtr->subId, sPtr->p, sPtr->x, w);//FIXME
    }
...
//                                     sPtr->subId, sPtr->p, w);//FIXME
  ...
  for (idx=0, ptId=0; ptId < numSourcePts; ptId++)
    ...
    if ( sPtr->cellId >= 0 ) //starting point in dataset
      {
//      cell = input->GetCell(sPtr->cellId);//FIXME
//      cell->EvaluateLocation(sPtr->subId, sPtr->p, xNext, w);//FIXME
...
//  input->GetCell(0,gcell);//FIXME
  gcell->Delete();
  
  // Set up and execute the thread
  this->Threader->SetNumberOfThreads( this->NumberOfThreads );
  this->Threader->SetSingleMethod( vtkGenericStreamer::ThreadedIntegrate, (void *)this );
...
void vtkGenericStreamer::PrintSelf(ostream& os, vtkIndent indent)
{
