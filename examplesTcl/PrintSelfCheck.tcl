...
# Fileid for the PrintSelfResults.html file
set pr_id 0
...
proc check_header_file { filename } {
  ...
  if { [file readable $filename] } {
    ...
    while { [gets $fileid data] >= 0 && $protected_not_found } {
      ...
      if { [string match "*class VTK_EXPORT*" $data] == 1 } {
        set class_found 1
...
proc print_toolkit_results { toolkit } {
  ...
  puts $pr_id " "
  if { [string length $toolkit] < 8 } { 
    puts $pr_id "$tk\t\t$class_count\t$class_print_count\t$super_miss_count\t$printself_miss_count\t$ivar_count\t$ivar_miss_count"
  } else {
    puts $pr_id "$tk\t$class_count\t$class_print_count\t$super_miss_count\t$printself_miss_count\t$ivar_count\t$ivar_miss_count"
  }
...
proc open_files { } {
  ...
  set pr_id [open "PrintSelfResults.html" "w" ]

  set pd_id [open "PrintSelfDetails.html" "w" ]
