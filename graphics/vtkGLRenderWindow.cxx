...
vtkGLRenderWindow::vtkGLRenderWindow()
{
  this->Gid = -2;
  this->MultiSamples = 8;
...
void vtkGLRenderWindow::Start(void)
{
...
void vtkGLRenderWindow::Frame(void)
{
...
void vtkGLRenderWindow::StereoUpdate(void)
{
...
void vtkGLRenderWindow::WindowConfigure()
{
...
void vtkGLRenderWindow::WindowInitialize (void)
{
  GLXconfig  *conf;
  XVisualInfo  *v;
...
void vtkGLRenderWindow::Initialize (void)
{
...
void vtkGLRenderWindow::Connect()
{
  int status = -1;
...
void vtkGLRenderWindow::SetFullScreen(int arg)
{
  int *temp;
...
void vtkGLRenderWindow::PrefFullScreen()
{
  this->Connect();
...
// Description:
// Resize the window.
void vtkGLRenderWindow::WindowRemap()
{
  short cur_light;
...
void vtkGLRenderWindow::SetSize(int x,int y)
{
...
int vtkGLRenderWindow::GetDesiredDepth()
{
  GLXconfig *conf;
  XVisualInfo *v;
...
Visual *vtkGLRenderWindow::GetDesiredVisual ()
{
  XVisualInfo  *v;
  GLXconfig *conf;
...
Colormap vtkGLRenderWindow::GetDesiredColormap ()
{
  GLXconfig *conf;
...
void vtkGLRenderWindow::PrintSelf(ostream& os, vtkIndent indent)
{
...
unsigned char *vtkGLRenderWindow::GetPixelData(int x1, int y1, int x2, int y2,
						int front)
{
  long     xloop,yloop;
  int     y_low, y_hi;
...
void vtkGLRenderWindow::SetPixelData(int x1, int y1, int x2, int y2,
...
float *vtkGLRenderWindow::GetRGBAPixelData(int x1, int y1, int x2, int y2,
					    int front)
{
  long    xloop,yloop;
  int     y_low, y_hi;
  ...
  buffer = new unsigned long[abs(x2 - x1)+1];
  data = new float[(abs(x2 - x1) + 1)*(abs(y2 - y1) + 1)*4];
  ...
  for (yloop = y_low; yloop <= y_hi; yloop++)
    ...
    for (xloop = 0; xloop <= (abs(x2-x1)); xloop++)
      {
      *p_data = (float)( buffer[xloop] & (0x000000ff))*scale; p_data++;
      *p_data = (float)((buffer[xloop] & (0x0000ff00)) >>  8)*scale; p_data++;
      *p_data = (float)((buffer[xloop] & (0x00ff0000)) >> 16)*scale; p_data++;
      *p_data = (float)((buffer[xloop] & (0xff000000)) >> 24)*scale; p_data++;
      }
    }
				     ...
				     float *data, int front)
{
  ...
  for (yloop = y_low; yloop <= y_hi; yloop++)
    ...
    for (xloop = 0; xloop <= (abs(x2-x1)); xloop++)
      {
      buffer[xloop] =  (unsigned char)(*p_data * 255.0); p_data++; 
      buffer[xloop] += (unsigned char)(*p_data * 255.0) << 8; p_data++;
      buffer[xloop] += (unsigned char)(*p_data * 255.0) << 16; p_data++;
      buffer[xloop] += (unsigned char)(*p_data * 255.0) << 24; p_data++;
      }
    /* write out a row of pixels */
    lrectwrite(x_low,yloop,x_hi,yloop,buffer);
    }
...
void vtkGLRenderWindow::SetZbufferData(int x1, int y1, int x2, int y2, 
					float *f_z_data )
{
  int    y_low, y_hi;
  int    x_low, x_hi;
...
float *vtkGLRenderWindow::GetZbufferData( int x1, int y1, int x2, int y2 )
{
  int    y_low, y_hi;
  int    x_low, x_hi;
  int    width, height;
