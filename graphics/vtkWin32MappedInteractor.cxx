/*=========================================================================
  ...
  Module:    vtkWin32MappedInteractor.cxx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
void  vtkWin32MappedInteractor::Start()
{
  // not necessary
}

void vtkWin32MappedInteractor::PrintSelf(ostream& os, vtkIndent indent)
{
...
void  vtkWin32MappedInteractor::UpdateSize(int x,int y)
{
...
void vtkWin32MappedInteractor::Initialize(HWND hwnd, RECT *rcBounds,vtkRenderWindow *renw)
{
  MakeDirectRenderer(hwnd, rcBounds,renw);
}

void vtkWin32MappedInteractor::MakeDirectRenderer(HWND hwnd, RECT *rcBounds,vtkRenderWindow *renw)
{
  BOOL bResult;
  int bitsperpixel;
  ...
  this->WindowWidth  = rcBounds->right - rcBounds->left;
  this->WindowHeight = rcBounds->bottom - rcBounds->top;
...
void vtkWin32MappedInteractor::MakeIndirectRenderer(int bitmap_width,int bitmap_height,int bitsperpixel,vtkRenderWindow *renw)
{
  void *m_pBits;
  ...
  if (this->MemoryDC != NULL) 
    ...
    this->WindowBitmap = CreateDIBSection(this->MemoryDC,
...
void vtkWin32MappedInteractor::OnMouseMove(HWND wnd,UINT nFlags, POINT& point) 
{
  this->LastPosition.x = point.x;
  this->LastPosition.y = point.y;	
}

void vtkWin32MappedInteractor::OnRButtonDown(HWND wnd,UINT nFlags, POINT& point) 
{
  SetCapture(wnd ); 
...
void vtkWin32MappedInteractor::OnRButtonUp(HWND wnd,UINT nFlags, POINT& point) 
{
  wglMakeCurrent(this->WindowDC,this->WindowRC);
...
void vtkWin32MappedInteractor::OnLButtonDown(HWND wnd,UINT nFlags, POINT& point) 
{
  SetCapture(wnd ); // nje
...
void vtkWin32MappedInteractor::OnLButtonUp(HWND wnd,UINT nFlags, POINT& point) 
{
...
void vtkWin32MappedInteractor::OnTimer(HWND wnd,UINT nIDEvent) 
{
  float xf,yf;
...
void vtkWin32MappedInteractor::OnChar(HWND wnd,UINT nChar, UINT nRepCnt, UINT nFlags) 
{
  wglMakeCurrent(this->WindowDC,this->WindowRC);
...
void vtkWin32MappedInteractor::Update()
{
...
HBITMAP vtkWin32MappedInteractor::GetBitmap()
{
  return this->WindowBitmap;
}

void vtkWin32MappedInteractor::BitBlt(HDC hDC,int x_position,int y_position)
{
	BOOL bValue = ::BitBlt(hDC, x_position, y_position, this->WindowWidth, 
			    this->WindowHeight, this->MemoryDC, 0, 0, SRCCOPY);
}

void vtkWin32MappedInteractor::DescribePixelFormat(HDC hDC,DWORD flags,int bitsperpixel)
{
...
void vtkWin32MappedInteractor::GetBitmapInfo(LPBITMAPINFOHEADER lpbi)
{
...
HDIB vtkWin32MappedInteractor::GetDIB(int width,int height,int bitsperpixel)
{
...
BOOL vtkWin32MappedInteractor::StretchDIB(HDC hDC,int x_position,int y_position, int x_width,int y_width,
...
BOOL vtkWin32MappedInteractor::SaveBMP(LPCTSTR lpszPathName,int width,int height,int bitsperpixel)
{
  BOOL bValue = FALSE;
  HWND hwnd = this->WindowHandle;
...
PBITMAPINFO vtkWin32MappedInteractor::CreateBitmapInfoStruct(HWND hwnd, HBITMAP hBmp) 
{ 
  BITMAP bmp; 
  PBITMAPINFO pbmi; 
  WORD    cClrBits; 
...
void vtkWin32MappedInteractor::CreateBMPFile(HWND hwnd, LPTSTR pszFile, 
				     PBITMAPINFO pbi, 
...
#ifdef TIMER
void vtkWin32MappedInteractor::StartTiming(int count)
{
  if(bAuto==FALSE) 
    {
    MiliSeconds=count;
...
void vtkWin32MappedInteractor::StopTiming()
{
  if(bAuto==TRUE) 
    {
    bAuto = FALSE;
