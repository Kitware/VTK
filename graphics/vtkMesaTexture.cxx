...
long vtkMesaTexture::GlobalIndex = 0;

// Initializes an instance, generates a unique index.
vtkMesaTexture::vtkMesaTexture()
{
  this->Index = 0;
  this->RenderWindow = 0;
}
...
void vtkMesaTexture::ReleaseGraphicsResources(vtkWindow *renWin)
{
  if (this->Index && renWin)
    ...
    if (glIsTexture(this->Index))
      ...
      // NOTE: Sun's Mesa seems to require disabling of texture before delete
...
void vtkMesaTexture::Load(vtkRenderer *ren)
{
  ...
  if (this->GetMTime() > this->LoadTime.GetMTime() ||
    ...
    // xsize and ysize must be a power of 2 in Mesa
    ...
    if ((xs > 1)||(ys > 1))
      {
      vtkDebugMacro(<< "Resampling texture to power of two for Mesa");
      resultData = this->ResampleToPowerOfTwo(xsize, ysize, dataPtr, 
                                              bytesPerPixel);
      }

    // format the data so that it can be sent to Mesa
...
#ifdef _WIN32
    ((vtkWin32MesaRenderWindow *)(ren->GetRenderWindow()))->RegisterTextureResource( this->Index );
#else
    ((vtkMesaRenderWindow *)(ren->GetRenderWindow()))->RegisterTextureResource( this->Index );
#endif
...
unsigned char *vtkMesaTexture::ResampleToPowerOfTwo(int &xs, int &ys, unsigned char *dptr,
                                                      int bpp)
{
