...
class_def : CLASS VTK_ID 
    ...
    optional_scope 
      ...
      if (!num_superclasses)
	{
	fprintf(yyout,"import java.beans.*;\n");
	}
    ...
    '{' class_def_body '}'
      {
        fprintf(yyout,"  public %s getThis%s() { return this;}\n\n",class_name,class_name+3);
	...
	if (!num_superclasses)
	  ...
	  /* beans */
	  fprintf(yyout,"  public void addPropertyChangeListener(PropertyChangeListener l)\n    {\n");
	  fprintf(yyout,"    changes.addPropertyChangeListener(l);\n    }\n");
	  fprintf(yyout,"  public void removePropertyChangeListener(PropertyChangeListener l)\n    {\n");
	  fprintf(yyout,"    changes.removePropertyChangeListener(l);\n    }\n");
	  fprintf(yyout,"  protected PropertyChangeSupport changes = new PropertyChangeSupport(this);\n\n");
...
output_function()
{
  ...
  if (in_public && args_ok)
	...
	return_result();
	fprintf(yyout,"%s(",beanfunc); 
	/* beans	fprintf(yyout,"%s(",func_name); */
	for (i = 0; i < num_args; i++)
	  {
	  if (i)
	    {
	...
	if (arg_types[10]%1000 == 2)
	  ...
	  if (!strncmp(beanfunc,"set",3) && num_args == 1 && 
	      (arg_types[0] < 10 || arg_types[0] == 303 || arg_types[0]%10 == 9))
	    {
	    char prop[256];
		...
		{
		case 1:
		case 7:   fprintf(yyout," new Double(id0)"); break;
		case 4:   
		case 5:   
		case 6:   fprintf(yyout," new Integer(id0)"); break;
		case 9:   fprintf(yyout," id0"); break;
		case 3:   /* not implemented yet */ 
		default:  fprintf(yyout," null");
		}
	      }
	    }
	  /* not a set method is it an On/Off method ? */
	    ...
	    if (!strncmp(beanfunc + strlen(beanfunc) - 2, "On",2))
	      {
	      /* OK we think this is a Boolean method so we need to fire a change */
	      char prop[256];
