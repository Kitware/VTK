...
void vtkParseOutput(FILE* fp, FileInfo* data)
{
  ...
  for (i = 0; i < data->NumberOfFunctions; i++)
  {
    if (data->Functions[i].Name && data->Functions[i].IsPublic)
    {
      ...
      {
        fprintf(fp, "    <li><a href=\"#method%i\">%s</a></li>", i, data->Functions[i].Signature);
      }
      else
      {
  ...
  for (i = 0; i < data->NumberOfFunctions; i++)
  {
    if (data->Functions[i].Name && !(data->Functions[i].IsProtected || data->Functions[i].IsPublic))
    {
      no_methods = 0;
      if (data->Functions[i].Comment)
      {
  ...
  for (i = 0; i < data->NumberOfFunctions; i++)
  {
    if (data->Functions[i].Name && data->Functions[i].Comment)
    {
      ...
      fprintf(fp, "    <li><a name=\"method%i\">%s%s</a></li>\n", i, data->Functions[i].Signature,
        ((data->Functions[i].IsPublic)
            ? ""
            : (data->Functions[i].IsProtected) ? " [protected]" : " [private]"));
        ...
        if (i == (data->NumberOfFunctions - 1) ||
          ...
          (i < (data->NumberOfFunctions - 1) &&
            ...
            (!data->Functions[i + 1].Comment ||
              ...
              (strcmp(data->Functions[i].Comment, data->Functions[i + 1].Comment)))))
        {
          closed = 1;
