...
static void CreateInitFile(const char* libName, int numDepends, char** depends, int doSerDes,
  int numFiles, char** files, FILE* fout)
{
  int i;
  ...
  if (doSerDes)
  {
    fprintf(fout, "#include \"%sSerDes.h\"\n", libName);
  }
  ...
  if (doSerDes)
  {
    fprintf(fout, "  if (auto* o = vtkPythonUtil::ManglePointer(\n");
    fprintf(fout, "        reinterpret_cast<void*>(RegisterClasses_%s), \"p_void\"))\n", libName);
    fprintf(fout, "  {\n");
    fprintf(fout,
      "    PyDict_SetItemString(d, \"RegisterClasses_%s\", "
      "PyUnicode_FromString(o));\n",
      libName);
    fprintf(fout, "    Py_DECREF(o);\n");
    fprintf(fout, "  }\n");
  }
...
int VTK_PARSE_MAIN(int argc, const char* argv[])
{
  ...
  {
    if (strcmp(tmpVal, "INCLUDE_MARSHAL") == 0)
    {
      doSerDes = 1;
    }
    else if (strcmp(tmpVal, "DEPENDS") == 0)
    {
      doDepends = 1;
    }
    else if (doDepends)
  ...
  CreateInitFile(libName, numDepends, depends, doSerDes, numFiles, files, fout_init);
