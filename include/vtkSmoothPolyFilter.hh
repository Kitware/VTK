...
// line).
// 
// Another interesting ivar is FeatureEdgeSmoothing. If this ivar is
// enabled, then interior vertices are classified as either "simple",
// "interior edge", or "corner", and smoothed differently. (Interior
// vertices are manifold vertices surrounded by a cycle of polygons, or used
// by two edges.) The classification is based on the number of feature edges
// attached to v. A feature edge occurs when the angle between the two
// surface normals of a polygon sharing an edge is greater than
// FeatureAngle.  Then, vertices used by no feature edges are classified
// "simple", vertices used by exactly two feature edges are classified
// "interior edge", and all others are "corner" vertices.
//
...
public:
  ...
  // Description:
  // Specify the expansion factor for Laplacian smoothing,
  vtkSetClampMacro(ExpansionFactor,float,0.0,1.0);
  vtkGetMacro(ExpansionFactor,float);

  // Description:
  // Specify the contraction factor for Laplacian smoothing, If this term is
  // non-zero, then the expansion pass is followed by a contraction pass. 
  // Using contraction in combination with expansion tends to preserve the 
  // original "volume" of the mesh.
  vtkSetClampMacro(ContractionFactor,float,0.0,-1.0);
  ...
  // or boundary).
  vtkSetClampMacro(EdgeAngle,float,0.0,90.0);
