...
/// with a user-created data model). This class only works with ADIOS2 files.
...
public:
  ...
  /// Setup this instance with an already created ADIOSDataSource
  InternalMetadataSource(std::shared_ptr<fides::io::ADIOSDataSource> source);
...
private:
  std::shared_ptr<fides::io::ADIOSDataSource> Source = nullptr;
};
