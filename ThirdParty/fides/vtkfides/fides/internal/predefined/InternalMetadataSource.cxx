...
std::string ReadSingleValue(std::shared_ptr<fides::io::ADIOSDataSource> source,
                            const std::string& attrName)
{
  ...
  if (attr.size() != 1)
  {
    throw std::runtime_error("Fides was not able to read " + attrName + " from " +
                             source->FileName);
  }
  return attr[0];
}
...
{
  this->Source.reset(new fides::io::ADIOSDataSource());
...
InternalMetadataSource::InternalMetadataSource(std::shared_ptr<fides::io::ADIOSDataSource> source)
  : Source(source)
{
}
