...
void Field::ProcessJSON(const rapidjson::Value& json, DataSourcesType& sources)
{
           ...
           json["variable_list_attribute_name"].IsString())
  {
    ...
    {
      if (source.second->GetBackendType() != fides::io::DataSourceBackend::ADIOS)
      {
        throw std::runtime_error("Wildcard fields are only supported by the ADIOS2 backend");
      }
    }
  ...
  if (json.HasMember("association") && json["association"].IsString())
  {
    ...
    if (assoc == "points")
    {
      this->Association = fides::FieldAssociation::Points;
    }
    else if (assoc == "cell_set")
    {
      this->Association = fides::FieldAssociation::Cells;
    }
    else if (assoc == "field_data")
    {
      this->Association = fides::FieldAssociation::WholeDataSet;
    }
    else
    {
...
std::vector<size_t> Field::Read(const std::unordered_map<std::string, std::string>& paths,
                                DataSourcesType& sources,
...
void Field::CollectReadRequests(const std::unordered_map<std::string, std::string>& paths,
                                DataSourcesType& sources,
                                const fides::metadata::MetaData& selections,
...
std::vector<size_t> Field::EmitTokens(const std::unordered_map<std::string, std::string>& paths,
                                      DataSourcesType& sources,
                                      const fides::metadata::MetaData& selections,
...
void Field::PostRead(fides::DataContainer& container, const fides::metadata::MetaData& selections)
{
  this->Array->PostRead(container, selections);
}
