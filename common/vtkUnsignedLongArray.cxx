...
void vtkUnsignedLongArray::SetArray(unsigned long* array, int size, int save)
{
...
int vtkUnsignedLongArray::Allocate(const int sz, const int ext)
{
  if ( sz > this->Size )
    ...
    this->Size = ( sz > 0 ? sz : 1);
    if ( (this->Array = new unsigned long[this->Size]) == NULL ) return 0;
    this->SaveUserArray = 0;
    }
...
//
unsigned long *vtkUnsignedLongArray::Resize(const int sz)
{
...
void vtkUnsignedLongArray::InsertTuple(const int i, const float * tuple)
{
  ...
  for (int j=0; j<this->NumberOfComponents; j++) *t++ = (unsigned long)*tuple++;
}
...
int vtkUnsignedLongArray::InsertNextTuple(const float * tuple)
{
  ...
  for (i=0; i<this->NumberOfComponents; i++) *t++ = (unsigned long)*tuple++;
