  ...
  vtkInformationVector *outputVector)
{
  ...
  // set to -1 if image bounds are reversed
  double flipsign[3];
  flipsign[0] = flipsign[1] = flipsign[2] = 1.0;
  ...
  if (this->Cropping)
    ...
    for (int k = 0; k < 3; k++)
      {
      // if bounds are reversed, flip the image
      if (bounds[2*k] > bounds[2*k+1])
        {
        flipsign[k] = -1.0;
        inOrigin[k] += inSpacing[k]*(inExt[2*k] + inExt[2*k+1]);
        inSpacing[k] = -inSpacing[k];
  ...
  switch (this->ResizeMethod)
    ...
    case vtkImageResize::OUTPUT_DIMENSIONS:
      ...
      for (int i = 0; i < 3; i++)
        ...
        this->IndexStretch[i] = flipsign[i];
        if (d != 0 && e != 0)
          {
