/*=========================================================================
  ...
  Module:    ImageResliceOriented.cxx
...
=========================================================================*/
// Test vtkImageReslice with oriented image as input
//
...
int ImageResliceOriented(int argc, char* argv[])
{
  ...
  vtkNew<vtkImageReslice> orient;
  orient->SetInputConnection(reader->GetOutputPort());
  orient->SetOutputDirection(direction);
  orient->Update();

  const double* checkDirection = orient->GetOutput()->GetDirectionMatrix()->GetData();
  for (int i = 0; i < 9; ++i)
  {
    if (checkDirection[i] != direction[i])
    {
      std::cout << "Direction incorrect after setting:" << std::endl;
      orient->GetOutput()->GetDirectionMatrix()->Print(std::cout);
      return EXIT_FAILURE;
    }
  }
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    reslice->SetInputConnection(orient->GetOutputPort());
