...
void vtkBSplineKernel::BSpline(F* kernel, int m, int n)
{
  ...
  vtkImageBSplineInternals::GetInterpolationWeights(weights, offset, order);
  ...
  int vtkNotUsed(interpolationMode))
{
  switch (dataType)
  {
    vtkTemplateAliasMacro(*interpolate = &(vtkImageBSplineInterpolate<F, VTK_TT>::BSpline));
    default:
      *interpolate = nullptr;
  }
}
  ...
  int scalarType, int vtkNotUsed(interpolationMode))
{
  switch (scalarType)
  {
    vtkTemplateAliasMacro(*summation = &(vtkImageBSplineRowInterpolate<F, VTK_TT>::BSpline));
    default:
      *summation = nullptr;
  }
}
