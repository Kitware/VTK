...
int vtkSurfaceReconstructionFilter::RequestInformation(vtkInformation* vtkNotUsed(request),
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  for (i = 0; i < COUNT; i++)
  {
    ...
    for (j = 0; j < p->neighbors->GetNumberOfIds(); j++)
    {
      p->costs[j] = 1.0 - fabs(vtkMath::Dot(p->n, surfacePoints[p->neighbors->GetId(j)].n));
    }
  }
  ...
  // --------------------------------------------------------------------------
  {
    ...
    vtkDebugMacro(<< "Created output volume of dimensions: (" << dim[0] << ", " << dim[1] << ", "
                  << dim[2] << ")");

    // initialise the output volume
    outInfo->Set(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), 0, dim[0] - 1, 0, dim[1] - 1, 0,
      dim[2] - 1);
    ...
    outInfo->Set(vtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), 0, dim[0] - 1, 0, dim[1] - 1, 0,
      dim[2] - 1);

    vtkFloatArray* newScalars =
      vtkArrayDownCast<vtkFloatArray>(output->GetPointData()->GetScalars());
    outInfo->Set(
      vtkDataObject::SPACING(), this->SampleSpacing, this->SampleSpacing, this->SampleSpacing);
