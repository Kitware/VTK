...
{
public:
  static vtkSWCallback* New() { return new vtkSWCallback; }
  void Execute(vtkObject* caller, unsigned long, void*) override
  {
...
int TestSplineWidget(int argc, char* argv[])
{
  ...
  vtkSmartPointer<vtkSplineWidget> spline = vtkSmartPointer<vtkSplineWidget>::New();
  ...
  spline->SetPlaneSource(static_cast<vtkPlaneSource*>(ipw->GetPolyDataAlgorithm()));
  ...
  vtkSmartPointer<vtkIPWCallback> ipwcb = vtkSmartPointer<vtkIPWCallback>::New();
  ipwcb->Spline = spline;
  ipwcb->Poly = poly;

  ipw->AddObserver(vtkCommand::InteractionEvent, ipwcb);

  vtkSmartPointer<vtkSWCallback> swcb = vtkSmartPointer<vtkSWCallback>::New();
  swcb->Poly = poly;
