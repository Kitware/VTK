...
// .SECTION Description
// vtkPolyLineRepresentation is a vtkWidgetRepresentation for a poly line.
// This 3D widget defines a poly line that can be interactively placed in a
// scene. The poly line has handles, the number of which can be changed, plus it
// can be picked on the poly line itself to translate or rotate it in the scene.
// Based on vtkSplineWidget
// .SECTION See Also
// vtkPolyLineWidget, vtkSplineWidget
...
class VTKINTERACTIONWIDGETS_EXPORT vtkPolyLineRepresentation : public vtkWidgetRepresentation
{
public:
  static vtkPolyLineRepresentation* New();
  vtkTypeMacro(vtkPolyLineRepresentation, vtkWidgetRepresentation);
...
//ETX
  ...
  // Description:
  // Force the poly line widget to be projected onto one of the orthogonal planes.
  // Remember that when the InteractionState changes, a ModifiedEvent is invoked.
  // This can be used to snap the poly line to the plane if it is originally
  ...
  // Description:
  // Grab the polydata (including points) that defines the poly line.  The
  ...
  // Description:
  // Set/Get the position of the poly line handles. Call GetNumberOfHandles
  ...
  // Description:
  // Control whether the poly line is open or closed. A closed poly line forms
  ...
  // user supplied subclasses of vtkPolyLine.
  void SetClosed(int closed);
  vtkGetMacro(Closed,int);
  vtkBooleanMacro(Closed,int);

  // Description:
  // Convenience method to determine whether the poly line is
  // closed in a geometric sense.  The widget may be set "closed" but still
  // be geometrically open (e.g., a straight line).
  int IsClosed();

  // Description:
  // Get the approximate vs. the true arc length of the poly line. Calculated as
  ...
  // instance.  If the first and last points are the same, the poly line sets
...
protected:
  ...
  // Methods to manipulate the poly line.
