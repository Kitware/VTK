...
public:
  ...
  static void Dot(vtkVectorDot* self, vtkIdType numPts, TN* normals, TV* vectors, float* scalars,
  ...
  public:
    vtkVectorDotAlgorithm* Algo;
    MapOp(vtkVectorDotAlgorithm<T1, T2>* algo) { this->Algo = algo; }
    void operator()(vtkIdType k, vtkIdType end)
    {
...
template <class TN, class TV>
vtkVectorDotAlgorithm<TN, TV>::vtkVectorDotAlgorithm()
  : Normals(nullptr)
  , Vectors(nullptr)
  , Scalars(nullptr)
{
...
template <class TN, class TV>
void vtkVectorDotAlgorithm<TN, TV>::Dot(vtkVectorDot* self, vtkIdType numPts, TN* normals,
  TV* vectors, float* scalars, double range[2], double actualRange[2])
{
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  switch (vtkTemplate2PackMacro(inNormals->GetDataType(), inVectors->GetDataType()))
  {
    // Double explicit specification of multiple template arguments.
    // Supports combinations of float and double.
    vtkTemplate2MacroFP((vtkVectorDotAlgorithm<VTK_T1, VTK_T2>::Dot(
      this, numPts, (VTK_T1*)normals, (VTK_T2*)vectors, scalars, range, actualRange)));
  ...
  if (!fastPath)
  {
    ...
    for (min = VTK_DOUBLE_MAX, max = (-VTK_DOUBLE_MAX), ptId = 0; ptId < numPts && !abort; ptId++)
    {
