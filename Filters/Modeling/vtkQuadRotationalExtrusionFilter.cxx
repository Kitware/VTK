  ...
  vtkPoints* inPts, vtkPoints* newPts, vtkPointData* pd, vtkPointData* outPD)
{
  ...
  {
    vtkErrorMacro(<< "Invalid axis number: " << this->Axis << "\n");
    return 0;
  }
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  while (inputIterator->IsDoneWithTraversal() == 0)
  {
    ...
    if (numPts > 0 && numCells > 0)
    {
      ...
      vtkDebugMacro(<< "process block " << blockId << ", angle=" << blockAngle
...
void vtkQuadRotationalExtrusionFilter::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  for (std::map<vtkIdType, double>::iterator it = this->PerBlockAngles.begin();
