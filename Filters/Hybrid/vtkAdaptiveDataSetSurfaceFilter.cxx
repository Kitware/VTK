                                                     ...
                                                     vtkPolyData* output )
{
  ...
  if ( this->ViewPointDepend && this->ParallelProjection && this->Renderer )
  {
    ...
    // JB Le calcul qui suit a pour objet de determiner le niveau de parcour en profondeur utile pour l'affichage
    // JB en fonction de la distance
    if ( isInit )
    {
      // JB Taille Moyenne d'une maille du niveau 0 dans les coordonnees reelles suivant chaque direction
      double worldCellAverageScaleAxis1 = ( bounds[2 * this->Axis1 + 1] - bounds[2 * this->Axis1] ) / (double) ( gridSize[this->Axis1] ) / this->Scale;
      double worldCellAverageScaleAxis2 = ( bounds[2 * this->Axis2 + 1] - bounds[2 * this->Axis2] ) / (double) ( gridSize[this->Axis2] ) / this->Scale;

      // JB Taille de la fenetre dans les coordonnees reelles (GetParallelScale) suivant chaque direction
      double worldWindScaleAxis1 = cam->GetParallelScale() * this->LastRendererSize[0] / (double) ( this->LastRendererSize[1] );
      ...
      // JB 1) application du theoreme de Thales ; le rapport taille d'une maille de niveau 0 par la taille de la fenetre est identique si le
      // JB calcul se fait en coordonnees reelles ou en coordonnees ecran (pixel)
      // JB 2) la taille d'une maille de niveau L vaut la taille d'une maille de niveau 0 divisee par le facteur de raffinement eleve a la puissance L
      // JB 3) au final, le calcul qui suit a pour objet de determiner quand une maille fera un pixel
      double levelMaxiAxis1 = ( log ( windScaleAxis1 ) + log ( worldCellAverageScaleAxis1 ) - log ( worldWindScaleAxis1 ) ) / log ( f );
      double levelMaxiAxis2 = ( log ( windScaleAxis2 ) + log ( worldCellAverageScaleAxis2 ) - log ( worldWindScaleAxis2 ) ) / log ( f );
    ...
    // JB Par option, on peut fixer le niveau max independemment du calcul dynamique realise precedemment
    if ( this->FixedLevelMax >= 0 )
    {
      this->LevelMax = this->FixedLevelMax;
    }

    // JB Le calcul qui suit a pour objet de determiner le rayon du cercle dans les coordonnees reelles incluant la projection
    // JB de la fenetre. L'activation de CircleSelection permettra de ne produire que les mailles intersectant ce cercle centre
    ...
    // JB Le calcul qui suit a pour objet de determiner la boite englobante dans les coordonnees reelles (et sans tenir compte
    // JB d'un point de vue qui aurait tourne) incluant la projection de la fenetre. L'activation de BBSelection permettra de ne
    ...
    std::cerr << "Bounds X        " << this->WindowBounds[0] << " : " << this->WindowBounds[1] << std::endl;
    std::cerr << "       Y        " << this->WindowBounds[2] << " : " << this->WindowBounds[3] << std::endl;
#endif
  }
  else
  {
...
#ifndef NDEBUG
  std::cerr << "vtkAdaptiveDataSetSurfaceFilter #Points            " << this->Points->GetNumberOfPoints() << std::endl;
  std::cerr << "                                #Cells             " << this->Cells->GetNumberOfCells() << std::endl;
  std::cerr << "                                #Type&Connectivity " << this->Cells->GetNumberOfConnectivityEntries() << std::endl;
  std::cerr << "                          Cells #NbRejectByBB      " << this->NbRejectByBB << std::endl;
  std::cerr << "                                #NbRejectByCircle  " << this->NbRejectByCircle << std::endl;
#endif
  std::cerr << "vtkAdaptiveDataSetSurfaceFilter #Points            " << this->Points->GetNumberOfPoints() << std::endl;
  std::cerr << "                                #Cells             " << this->Cells->GetNumberOfCells() << std::endl;
  std::cerr << "                                #Type&Connectivity " << this->Cells->GetNumberOfConnectivityEntries() << std::endl;
  ...
  int level )
{
  ...
  if( ! insideBB && ( this->CircleSelection || this->BBSelection ) )
  {
    ...
    if ( this->CircleSelection )
    {
      ...
      insideBB = (
        pow( originAxis1 + half - this->LastCameraFocalPoint[this->Axis1], 2 ) +
...
//----------------------------------------------------------------------------
void vtkAdaptiveDataSetSurfaceFilter::ProcessLeaf1D( vtkHyperTreeGridNonOrientedGeometryCursor* cursor )
{
...
//----------------------------------------------------------------------------
void vtkAdaptiveDataSetSurfaceFilter::ProcessLeaf2D( vtkHyperTreeGridNonOrientedGeometryCursor* cursor )
