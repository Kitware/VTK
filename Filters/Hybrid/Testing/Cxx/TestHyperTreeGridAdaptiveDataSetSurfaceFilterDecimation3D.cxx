...
bool TestSimpleDecimation()
{
  ...
  double point[3] = { camera->GetFocalPoint()[0], camera->GetFocalPoint()[1],
    camera->GetFocalPoint()[2] };
  double position[3] = { camera->GetPosition()[0], camera->GetPosition()[1],
    camera->GetPosition()[2] };

  point[0] -= 1.5;
  position[0] -= 1.5;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 446))
  {
    return false;
  }
  ...
  point[0] -= 1.5;
  position[0] -= 1.5;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 267))
  {
    return false;
  }
  ...
  point[0] += 6;
  position[0] += 6;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 389))
  {
    return false;
  }
  ...
  point[1] += 1;
  position[1] += 1;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 382))
  {
    return false;
  }
  ...
  point[1] -= 3;
  position[1] -= 3;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 227))
  {
    return false;
  }
...
bool TestMaskedDecimation()
{
  ...
  point[0] -= 3;
  position[0] -= 3;
  if (!RenderAndCheckVisibleCells(camera, renWin, point, position, surface, pd, 414))
  {
    return false;
  }
  return true;
...
int TestHyperTreeGridAdaptiveDataSetSurfaceFilterDecimation3D(
