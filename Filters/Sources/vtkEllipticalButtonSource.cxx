  ...
  vtkInformationVector** vtkNotUsed(inputVector), vtkInformationVector* outputVector)
{
  ...
  int numPts =
    1 + this->CircumferentialResolution * (this->TextureResolution + this->ShoulderResolution + 1);
  if (this->TwoSided)
  {
    numPts *= 2;
  }
  ...
  newPolys->AllocateEstimate(
    this->CircumferentialResolution * (this->TextureResolution * this->ShoulderResolution), 1);
  ...
  int hRes = static_cast<int>(ceil(this->CircumferentialResolution * (dY / (dY + dX)))) / 2;
  hRes = (hRes <= 0 ? 1 : hRes);
  int wRes = (this->CircumferentialResolution - 2 * hRes) / 2;

  // Create the center point
  newPts->SetPoint(0, this->Center[0], this->Center[1], this->Center[2] + this->Depth);
  ...
  {
    this->InterpolateCurve(1, newPts, this->CircumferentialResolution, normals, tcoords,
      this->TextureResolution, 0, 0, offset, 1, 1, 1);
    this->CreatePolygons(newPolys, this->CircumferentialResolution, this->TextureResolution - 1, 1);
  }
  ...
  int c2Start = offset + (this->ShoulderResolution + 1) * this->CircumferentialResolution;
  for (i = 0; i < this->CircumferentialResolution; i++)
  {
    // compute the angle
  ...
  this->InterpolateCurve(0, newPts, this->CircumferentialResolution, normals, tcoords,
    this->ShoulderResolution, c1Start, 1, c2Start, 1, c1Start + this->CircumferentialResolution, 1);
  this->CreatePolygons(
    newPolys, this->CircumferentialResolution, this->ShoulderResolution, c1Start);
  ...
  int c2Incr, int startPt, int incr)
{
  ...
  for (i = 0; i < numPts; i++, c1StartPt += c1Incr, c2StartPt += c2Incr, startPt += incr)
  {
