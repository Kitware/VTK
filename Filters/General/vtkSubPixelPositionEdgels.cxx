...
int vtkSubPixelPositionEdgels::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  vtkStructuredPoints* gradMaps =
    vtkStructuredPoints::SafeDownCast(gradMapsInfo->Get(vtkDataObject::DATA_OBJECT()));
  ...
  {
    DMapData =
      vtkArrayDownCast<vtkDoubleArray>(gradMaps->GetPointData()->GetScalars())->GetPointer(0);
  }
  else if (vtkArrayDownCast<vtkFloatArray>(gradMaps->GetPointData()->GetScalars()))
  {
    MapData =
      vtkArrayDownCast<vtkFloatArray>(gradMaps->GetPointData()->GetScalars())->GetPointer(0);
  }
  else
  {
...
void vtkSubPixelPositionEdgels::Move(int xdim, int ydim, int zdim, int x, int y, double* img,
  vtkDataArray* inVecs, double* result, int z, double* spacing, double* resultNormal)
{
  int ypos;
  vtkIdType zpos;
  double vec[3];
...
void vtkSubPixelPositionEdgels::Move(int xdim, int ydim, int zdim, int x, int y, float* img,
  vtkDataArray* inVecs, double* result, int z, double* spacing, double* resultNormal)
{
  int ypos;
  vtkIdType zpos;
...
vtkStructuredPoints* vtkSubPixelPositionEdgels::GetGradMaps()
{
  ...
  return vtkStructuredPoints::SafeDownCast(this->GetExecutive()->GetInputData(1, 0));
}

int vtkSubPixelPositionEdgels::FillInputPortInformation(int port, vtkInformation* info)
{
  if (port == 1)
  {
