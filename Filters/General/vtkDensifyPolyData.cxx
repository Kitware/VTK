...
public:
  ...
  vtkDensifyPolyDataInternals(double* verts, vtkIdType numVerts, const vtkIdType* vertIds,
    vtkIdType& numPoints, unsigned int nSubdivisions)
  {
  ...
  public:
    ...
    //
    Polygon(double* p, vtkIdType nPts, const vtkIdType* ptIds, vtkIdType nParentPoints = 0,
      vtkIdType* parentPtIds = nullptr)
    {
  ...
  vtkIdType* GetNextCell(vtkIdType& numVerts /* number of verts in the returned ids */)
  {
  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  {
    vtkWarningMacro("vtkDensifyPolyData has no points/cells to linearly interpolate.");
    return 0;
  }
  ...
  { // for every cell
    ...
    else
    {
      ...
      outputNumCells +=
        (npts * static_cast<vtkIdType>(pow(3.0, static_cast<int>(nSubdivisions - 1))));
      ...
      vtkDensifyPolyDataInternals polygons(p, npts, ptIds, outputNumPoints, nSubdivisions);
      ...
      {
        vtkIdType newCellId = outputPolys->InsertNextCell(numNewCellVerts, newCellVertIds);
        outputCD->CopyData(inputCD, cellId, newCellId);
      }
    } // else
    if (cellType == VTK_POLYGON)
