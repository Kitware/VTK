...
class PointSource(VTKPythonAlgorithmBase):
    def __init__(self): VTKPythonAlgorithmBase.__init__(self,
        nInputPorts=0,
...
stats.Update()
avg = stats.GetOutput().GetPointData().GetArray("scalar_average").GetValue(0)
max = stats.GetOutput().GetPointData().GetArray("scalar_maximum").GetValue(0)
min = stats.GetOutput().GetPointData().GetArray("scalar_minimum").GetValue(0)
stddev = stats.GetOutput().GetPointData().GetArray("scalar_stddev").GetValue(0)

stats = vtk.vtkTemporalStatistics()
stats.StreamTimeStepsOff()
stats.SetInputConnection(ps.GetOutputPort())
