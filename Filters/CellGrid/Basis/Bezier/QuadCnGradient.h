// Gradients of the arbitrary-order Bernstein-Bezier basis on the quadrilateral.
//
...
for (int pp = 0; pp <= order[1]; ++pp)
{
  ...
  {
    basisGradient[idx++] = drtmp[oo] * stmp[pp]; // d/dr
    basisGradient[idx++] = rtmp[oo] * dstmp[pp]; // d/ds
    basisGradient[idx++] = 0.;
  }
}
