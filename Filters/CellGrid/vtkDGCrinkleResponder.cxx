...
vtkStandardNewMacro(vtkDGCrinkleResponder);

bool vtkDGCrinkleResponder::Query(
  vtkCellGridCrinkleQuery* query, vtkCellMetadata* cellType, vtkCellGridResponders* caches)
{
  (void)caches;
  switch (query->GetPass())
  {
    case vtkCellGridCrinkleQuery::PassWork::HashSides:
      return this->HashCrinkle(query, dynamic_cast<vtkDGCell*>(cellType));
    case vtkCellGridCrinkleQuery::PassWork::Summarize:
      return this->SummarizeCrinkle(query, dynamic_cast<vtkDGCell*>(cellType));
    case vtkCellGridCrinkleQuery::PassWork::GenerateSideSets:
      return this->GenerateSideSets(query, dynamic_cast<vtkDGCell*>(cellType));
  }
  return false;
}

bool vtkDGCrinkleResponder::HashCrinkle(vtkCellGridCrinkleQuery* query, vtkDGCell* cellType)
{
  ...
  if (!cellSpec.Blanked)
  {
    ...
    for (vtkIdType ii = 0; ii < conn->GetNumberOfTuples(); ++ii)
    {
      ...
      for (int sideType = 0; sideType < numSideTypes; ++sideType)
      {
        ...
        if (!vtkDGCrinkleResponder::ProcessCrinkleOfInput(query, shape, cellSpec.SourceShape))
        {
  ...
  for (std::size_t ii = 0; ii < numInputSideSpecs; ++ii)
  {
    ...
    for (vtkIdType jj = 0; jj < sides->GetNumberOfTuples(); ++jj)
    {
      sides->GetIntegerTuple(jj, sideEntry.data());
      ...
      this->HashCrinkleOfSide(query, cellType, sideSpec.SourceShape, side, sidesOfSide,
        sideEntry[0], entry, sidesVisited, ngm);
    }
  }
...
bool vtkDGCrinkleResponder::SummarizeCrinkle(vtkCellGridCrinkleQuery* query, vtkDGCell* cellType)
{
  ...
  for (auto nx = it; it != hashes.end(); it = nx)
  {
    ...
    if (firstSide.CellType == cellTypeToken)
    {
      ...
      {
        case vtkCellGridCrinkleQuery::Winding:
        ...
        case vtkCellGridCrinkleQuery::SummaryStrategy::AnyOccurrence:
          // Always consume and output the hash.
          break;

        case vtkCellGridCrinkleQuery::SummaryStrategy::Boundary:
...
bool vtkDGCrinkleResponder::GenerateSideSets(vtkCellGridCrinkleQuery* query, vtkDGCell* cellType)
{
  auto* grid = cellType ? cellType->GetCellGrid() : nullptr;
  if (!grid)
  {
  ...
  for (const auto& sideSetArray : sideSetArrays)
  {
    ...
    sideSpecs.back().SelectionType =
      query->GetSelectionType() == vtkCellGridCrinkleQuery::SelectionMode::Input
      ? -1
...
bool vtkDGCrinkleResponder::ProcessCrinkleOfInput(
  vtkCellGridCrinkleQuery* query, vtkDGCell::Shape sideShape, vtkDGCell::Shape inputShape)
{
    ...
    case 1: // edges
      switch (sideDim)
      {
        case 0: // vertices of edges
          return outputControl & vtkCellGridCrinkleQuery::VerticesOfEdges;
        default:
          return false;
      }
    case 2:
      switch (sideDim)
      {
        case 0: // vertices of a surface
          return outputControl & vtkCellGridCrinkleQuery::VerticesOfSurfaces;
        case 1: // edges of a surface
          return outputControl & vtkCellGridCrinkleQuery::EdgesOfSurfaces;
        default:
          return false;
      }
    case 3:
      ...
      {
        case 0:
          return outputControl & vtkCellGridCrinkleQuery::VerticesOfVolumes;
        case 1:
          return outputControl & vtkCellGridCrinkleQuery::EdgesOfVolumes;
        case 2:
          return outputControl & vtkCellGridCrinkleQuery::SurfacesOfVolumes;
        default:
          return false;
      }
  }
...
void vtkDGCrinkleResponder::HashCrinkleOfSide(vtkCellGridCrinkleQuery* query, vtkDGCell* cellType,
  ...
  const std::vector<vtkTypeInt64>& entry, std::set<int>& sidesVisited, vtkDataArray* ngm)
{
  ...
  for (const auto& sideId : sidesToHash)
  {
    ...
    if (vtkDGCrinkleResponder::ProcessCrinkleOfInput(query, sideOfSideShape, sourceShape))
    {
    ...
    const auto& childCrinkle = cellType->GetSidesOfSide(sideId);
    if (!childCrinkle.empty())
    {
      this->HashCrinkleOfSide(
        query, cellType, sideOfSideShape, side, childCrinkle, cellId, entry, sidesVisited, ngm);
    }
  }
}
