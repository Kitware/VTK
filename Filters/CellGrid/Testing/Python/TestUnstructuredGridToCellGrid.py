...
class TestUnstructuredGridToCellGrid(Testing.vtkTest):

    def runConverter(self, filename, fieldname, time, baseline, expectedUGridCells, expectedCGridCells, expectedBounds):
        ...
        converter = fc.vtkUnstructuredGridToCellGrid()
        converter.SetInputConnection(reader.GetOutputPort())
        converter.Update()
        cgrid = converter.GetOutputDataObject(0)
    ...
    def testCellData(self):
        filename = os.path.join(Testing.VTK_DATA_ROOT, 'Data', 'can.ex2')
        # Bounds @ t0
        # bds = [ \
        ...
        self.runConverter(filename, 'EQPS', 0.001, 'TestUnstructuredGridToCellGrid-DG-C0.png', 7880, 7880, bds)

    def testPointData(self):
        filename = os.path.join(Testing.VTK_DATA_ROOT, 'Data', 'disk_out_ref.ex2')
        ...
        self.runConverter(filename, 'AsH3', 0.0, 'TestUnstructuredGridToCellGrid-CG-C1.png', 10427, 10425, bds)

if __name__ == "__main__":
    Testing.main([(TestUnstructuredGridToCellGrid, 'test')])
