...
double vtkAxisExtended::Simplicity(
  int qIndex, int qLength, int j, double lmin, double lmax, double lstep)
{
...
{
  double coverage =
    1.0 - 0.5 * (pow(dmax - lmax, 2) + pow(dmin - lmin, 2) / pow(0.1 * (dmax - dmin), 2));
  return coverage;
}
...
double vtkAxisExtended::Legibility(
  double lmin, double lmax, double lstep, double scaling, vtkVector<int, 3>& parameters)
{
  ...
  for (int iFormat = 1; iFormat < 9; ++iFormat)
  {
    ...
    for (int fontIndex = 0; fontIndex < 8; ++fontIndex)
    {
      ...
      {
        fontLegSum = 0.2 * (iFont - fontSizes[0] + 1) / (this->DesiredFontSize - fontSizes[0]);
      }
      else
      {
        fontLegSum = -100.0;
      ...
      for (int iOrientation = 0; iOrientation < 2; ++iOrientation)
      {
        ...
        if (legScore > bestLegScore)
        {
          ...
          {
            double fontExtent;
            if ((this->IsAxisVertical && iOrientation) || (!this->IsAxisVertical && !iOrientation))
            {
              fontExtent =
                (FormatStringLength(iFormat, tickPositions[numTicks - 1], this->Precision) +
                  FormatStringLength(iFormat, tickPositions[numTicks - 2], this->Precision)) *
                iFont;
            }
            else
            {
          ...
          legScore = (formatLegSum + fontLegSum + orientLegSum + overlapLegSum) / 4;
...
vtkVector3d vtkAxisExtended::GenerateExtendedTickLabels(
  double dmin, double dmax, double m, double scaling)
{
      ...
      while (k < INF)
      {
        ...
        while (z < INF)
        {
          ...
          for (int start = minStart; start <= maxStart; ++start)
          {
            ...
            double newScore = this->Legibility(lmin, lmax, lstep, scaling, legibilityIndex);
