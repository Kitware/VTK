...
vtkCxxRevisionMacro(vtkGeoTerrainCache, "1.1");
...
//----------------------------------------------------------------------------
vtkGeoTerrainCache::vtkGeoTerrainCache() 
{
  ...
  this->ThreadId = this->Threader->SpawnThread( vtkGeoTerrainCacheThreadStart, this);
}
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkGeoTerrainCache::SetTerrainSource(vtkGeoTerrainSource* source)
{
  ...
  //writer->SetFileName("C:/Law/tmp/east.vtk");
  //writer->SetInput(this->EasternHemisphere->GetModel());
  //writer->Write();
  //writer->SetFileName("C:/Law/tmp/west.vtk");
  //writer->SetInput(this->WesternHemisphere->GetModel());
  //writer->Write();
  //writer->Delete();
}
...
int vtkGeoTerrainCache::RefineNode(vtkGeoTerrainNode* node)
{
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::InitializeTerrain(vtkGeoTerrain* terrain)
{
  terrain->StartEdit();
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::Request(vtkGeoTerrainNode* node, vtkGeoCamera* cam)
{
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::ThreadStart()
{
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::Request(vtkGeoCamera* camera)
{
  if (camera == 0)
    {
    return;
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::RequestTerminate()
{
  this->Camera = 0;
  this->WaitForRequestMutex1->Unlock();
  vtkSleep(0.01);
...
//-----------------------------------------------------------------------------
bool vtkGeoTerrainCache::GetReadLock()
{
  this->TreeMutex->Lock();
  if (this->TreeLock)
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::ReleaseReadLock()
{
  this->TreeMutex->Unlock();
}

//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::GetWriteLock()
{
  this->TreeMutex->Lock();
  this->TreeLock = 1;
  this->TreeMutex->Unlock();
...
//-----------------------------------------------------------------------------
void vtkGeoTerrainCache::ReleaseWriteLock()
{
  this->TreeMutex->Lock();
  this->TreeLock = 0;
  this->TreeMutex->Unlock();
...
//-----------------------------------------------------------------------------
bool vtkGeoTerrainCache::Update(vtkGeoTerrain* terrain, vtkGeoCamera* camera)
{
...
int vtkGeoTerrainCache::EvaluateNode(vtkGeoTerrainNode* node, vtkGeoCamera* cam)
{
