                ...
                vtkVolume *volume)
{
                        ...
                        || (this->InputAnalyzedTime < input->GetMTime()) )
        {
                ...
                for (i = 0; cells->GetNextCell(npts, pts); i++)
                {
                        ...
                        float size = static_cast<float>(vtkMath::Distance2BetweenPoints(min, max));
...
void vtkOpenGLProjectedAAHexahedraMapper::UpdatePreintegrationTexture(vtkVolume *volume, vtkDataArray *scalars)
{
        ...
        this->LengthScale = static_cast<float>((pi->GetIntegrationTableLengthResolution() - 2) / pi->GetIntegrationTableLengthScale());
  ...
  switch (inScalars->GetDataType())
    {
            vtkTemplateMacro(vtkOpenGLProjectedAAHexahedraMapperConvertScalars(
                               static_cast<const VTK_TT *>(inScalars->GetVoidPointer(0)),
  ...
  switch (inPoints->GetDataType())
    {
            vtkTemplateMacro(vtkOpenGLProjectedAAHexahedraMapperConvertPoints(
                               static_cast<const VTK_TT *>(inPoints->GetVoidPointer(0)),
                                                     ...
                                                     vtkVolume *volume)
{
        ...
        {
        this->UpdateProgress(static_cast<double>(numcellsrendered)/static_cast<double>(totalnumcells));
                if (renderer->GetRenderWindow()->CheckAbortStatus())
                {
                        break;
                }
                ...
                for (vtkIdType i = 0; i < num_cell_ids; i++)
                {
                        ...
                        float s = static_cast<float>((scalars[index] * this->ScalarScale + this->ScalarShift + 0.5)/this->ScalarResolution);
                        float mins = s;
                        float maxs = s;
                        ...
                        for(j = 0; j < 8; j++)
                        {
                                ...
                                s = static_cast<float>((scalars[index] * this->ScalarScale + this->ScalarShift + 0.5)/this->ScalarResolution);
                                if (s < mins) mins = s;
                                if (s > maxs) maxs = s;
