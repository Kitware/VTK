...
//    }
 ...
 compositeZPass->SetController(this->Controller);
  ...
  passes->AddItem(lights);
  passes->AddItem(opaque);
  passes->AddItem(compositeZPass); // just to see the png files.
...
//  AddLightActors(renderer);
  ...
  renderer->SetBackground2(157.0/255.0*0.66,186/255.0*0.66,192.0/255.0*0.66);
  renderer->SetGradientBackground(true);
  renWin->SetSize(100,100); // 400,400
  renWin->SetPosition(0, 460*me); // translate the window
  prm->SetRenderWindow(renWin);
  prm->SetController(this->Controller);
...
//    this->Controller->Barrier();
  ...
  while(i<numProcs)
    ...
    if(i==me)
      {
      cout << "****** before visibility change." << endl;
      cout << "me=" << me << endl;
      cout << " rectangleActor.mTime=" << rectangleActor->GetMTime() << endl;
      cout << " boxActor.mTime=" << boxActor->GetMTime() << endl;
      cout << " coneActor.mTime=" << coneActor->GetMTime() << endl;
      cout << " sphereActor.mTime=" << sphereActor->GetMTime() << endl;
      }
    ++i;
    }
#if 1
  // Make some actors visibles on node 0 only
  if(me==0)
    {
    rectangleActor->SetVisibility(false);
    boxActor->SetVisibility(false);
  ...
  // Make some actors visibles on node 1 only
  if(me==1)
    {
//    rectangleActor->SetVisibility(false);
//    boxActor->SetVisibility(false);
    ...
    rectangleActor->SetDebug(true);
    boxActor->SetDebug(true);
    coneActor->SetDebug(true);
    sphereActor->SetDebug(true);
    }
#endif
  i=0;
  while(i<numProcs)
    ...
    if(i==me)
      {
      cout << "****** after visibility change." << endl;
      cout << "me=" << me << endl;
      cout << " rectangleActor.mTime=" << rectangleActor->GetMTime() << endl;
      cout << " boxActor.mTime=" << boxActor->GetMTime() << endl;
      cout << " coneActor.mTime=" << coneActor->GetMTime() << endl;
...
//  sleep(10);
  ...
  cout << "before resting" << endl;
  
  int retVal = vtkTesting::Test(this->Argc, this->Argv, renWin, 10);
  
  
  cout << "after resting" << endl;
  ...
  if(me==0)
    ...
    if ( retVal == vtkRegressionTester::DO_INTERACTOR)
    {
    cout << "interactive mode me=" << me << endl;
    iren->Start();
    cout << "after interactive mode me=" << me << endl;
    }
    ...
    {
    cout << "satellite before start services me=" << me << endl;
    prm->StartServices(); // start listening other processes (blocking call).
    // receive return value from root process.
    cout << "satellite after start services me=" << me << endl;
    this->Controller->Receive(&retVal, 1, 0, MY_RETURN_VALUE_MESSAGE);
    cout << "satellite after receive me=" << me << endl;
    }
